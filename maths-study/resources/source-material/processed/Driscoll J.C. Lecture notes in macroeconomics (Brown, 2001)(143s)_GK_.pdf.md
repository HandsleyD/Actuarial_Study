---
normalized_id: shared-lecture-notes-driscoll-j-c-lecture-notes-in-macroeconomics-brown-2001-143s-gk
exam_code: SHARED
material_scope: driscoll j.c. lecture notes in macroeconomics (brown, 2001)(143s)_gk_.pdf
material_group: shared
document_type: lecture-notes
source_repo_path: resources/source-material/shared/Driscoll J.C. Lecture notes in macroeconomics (Brown, 2001)(143s)_GK_.pdf
conversion_method: pdftotext
qa_status: pending
---
# shared-lecture-notes-driscoll-j-c-lecture-notes-in-macroeconomics-brown-2001-143s-gk
...
which implies:                      r
                                ...
                                N=                                      (1.3)
                                       2F
    The amount withdrawn each time is PNY , so average nominal money holdings
are:
                                           r
                                  PY          YF
                            M=         =P                                  (1.4)
...
The agent derives (instantaneous) utility from two sources:
    ...
    • Holding real balances, denoted Mt
                                     Pt
...
choose to allocate these resources in three ways:
    ...
    • As new money holdings, with real value M
                                             Pt .
                                               t
...
ler equation. There is a key difference, however: the marginal utility of con-
...
following first-order conditions from
    the Hamiltonian:9
    First for m and c
  8 Since the growth rate of x = the growth rate of x - the growth rate of y.
                             y
...
through the transfer.10
     What’s the revenue from printing money? Money is issued at rate Ṁ ; the
real value of this is just Ṁ
...
growing (i.e.
   ...
   and:
    ...
    The condition that real balances per person not grow in the steady state
      ṁ
(i.e. m  = 0) also implies that the rate
...
transactions (nominal income). Note that this proposition implies that
                                  ...
                                  Ṁ   V̇   Ṗ   Ẏ
                                     +    =    +                                   (1.38)
                                  M    V    P    Y
which yields the result of the first sentence, assuming that V̇V = 0.
...
assume a unit income elasticity so that we may write it as M    P = L(i)Y . Then
...
13
  ...
  13 Why? well, Ṁ = g implies that M = M egt (differentiate the latter to check), so that
                M                        0
log(M ) = log(M0 ) + gt
...
printing money, it has to print it at a faster and faster rate.
    This can be seen by looking at the definition of seigniorage: Ṁ  Ṁ M
                                                                  P = M P The
...
ket clears provided the other two markets clear.
    Thus our two equilibrium conditions are:
                               ...
                               Y = C(Y − T ) + I(r) + G                                 (2.1)
   3 See an undergraduate text, such as Mankiw, chapter 2.
...
rule. Either way, solving out for dY obtains:
   ...
   where
                            ...
                            Γ=                                               (2.6)
                                    Lr (1 − C 0 ) + I 0 LY
   If the LM curve is nearly horizontal, so that LLYr is nearly zero, then approx-
              1
imately Γ = 1−C 0 , and
...
i∗ − E[ ėe ]. One can derive this by noting that if one invests $1 in an asset
...
small x, we have that: i = i∗ − e2e−e2
    ...
    For now, let’s look at the completely static case, and assume that E[ ėe ] = 0.
...
two steady-state conditions (namely, Ẏ = 0 and ė = 0) and plot them as
follows:((Figure 2.6).
...
supply. This shifts the ė = 0 schedule to the right. The new stable arm and new
...
π e + f (U ), where π e is expected inflation. In practice, it was assumed that πte =
...
expectations of inflation evolve over time in response to past inflation.
   ...
   1. The sticky wage story: Suppose workers had to set their wage some time
      ...
      P is realized will then be W
                                 P =ω P .
      If we assume that firms are on their labor demand curves, then LD =
                       Pe
      LD ( W       D
   ...
   2. Money illusion or worker misperception story: Suppose workers are igno-
      ...
      P > P e , labor supply will increase, the amount of labor hired will go up,
...
of the following form:
                                 π = π e + f (u) + ²                       (2.14)
    where π e is determined rationally, that is from the economic model into
...
clear. We again need to in principle consider two cases:
  ...
  1. W̄ is such that W̄
                     P is above the labor market clearing level

  2. W̄ is such that W̄
                     P is below the labor market clearing level.
...
function is then:
   ...
   After taking logs, this implies:
                         ...
                         lt =     ln(1 − γ) − (wt − pt )                  (2.35)
                                γ            γ
   Define α = γ1 ln(1 − γ) and β = γ1 . Then we can show that
...
to the private benefit, B+CB−A . Let’s try to figure out how big this is. Note
...
by summing over the demands for all consumers. This implies that:
   ...
   If we use the facts that IPi = PPi Yi + M i
                                                and that Y = i PPi Yi and define
      P                                P IiP
...
62CHAPTER 2. NOMINAL RIGIDITIES AND ECONOMIC FLUCTUATIONS
 ...
 7. Consider the following version of the IS-LM model which incorporates the
    ...
    steady-state value of q (i.e. where both q̇ and ẏ equal zero), cq − α1 < 0.
...
to do at his or her discretion. The first section will describe some of the tradi-
...
effects of the policy. Stabilization policy will only be effective when σz2 < σx2 ,
or the variance of the variable under policy is lower than the variance without
policy. From the definitions of X,Y and Z we see that σz2 = σx2 + σy2 + 2rxy σx σy ,
...
ample, the rule is to fix the growth rate of some policy instrument are examples
...
in which the effects of monetary policy on the economy aren’t known with
certainty. Suppose that it is known that output y and money m have the
following relationship:

                           yt = a(L)yt−1 + b(L)mt + ²t ,                        (3.2)
...
a0 and b(L) = b0 + b1 L. Then the optimal policy is to set mt = − bb10 mt−1 −
a0
 b0 yt−1 . If b1 > b0 , we have an unstable difference equation- mt will be increasing
in absolute value at an explosive rate. Policy each period will become larger
and larger, as at each period a “correction” is applied to last period’s policy.
...
by:
                         yt = a0 yt−1 + b0 mt + (b1 + v)mt−1 + ²t ,              (3.4)
where v is some constant.
   The resulting equation for output is:

                                  yt = vmt−1 + ²t                               (3.5)

   Note that under the assumptions above, mt is nonstationary, and is in fact
explosive: the growth rate of mt , mt − mt−1 , is growing.
   Hence the variance of yt , which is equal to v 2 var(mt−1 ) + var(²), will also
be growing with t.
   Now consider the policy of just setting mt = m̄ for all t. Then,

                       yt = a0 yt−1 + b0 m̄ + (b1 + v)m̄ + ²t ,                 (3.6)
68                                 CHAPTER 3. MACROECONOMIC POLICY

                          1
     so that var(yt ) = 1−a 2 var(²).
                            0
...
of parameter uncertainty.
   These phenomena reflect the problems associated with monetary policy’s
...
have announced. Even worse, it’s possible than even if I never implemented the
...
3.2.1    Fischer’s Model of the Benevolent, Dissembling Gov-
         ernment
...
some level of capital k1 and can save by accumulating capital, to level k2 . The
marginal product of labor is constant at level a and the marginal product of
capital is constant at level r. She has a time endowment n̄; second period labor
is denoted n2 .
    In the second period, some level of government spending may be provided,
which contributes directly to utility.
    Utility is:

           u(c1 , c2 , n2 , g2 ) = ln c1 + δ(ln c2 + α ln(n̄ − n2 ) + β ln g2 )    (3.7)
...
imposed in the second period on labor be denoted τ2 , and let the second-period
...
sumer’s utility (or, strictly speaking, indirect utility where we have inserted
the solutions to the consumer’s problem into the utility function) subject to the
government budget constraint:

                                  g2 = k2 (r − r2 ) + n2 aτ2                            (3.19)
...
c2 β. In principle, ex post the government might have liked to have simply used
...
ever, consumers might choose to not save any capital and instead supply more
     3 In an economy with more than two periods, this would be a time-inconsistent policy.
   4 Note that since the real interest rate is constant, this is equivalent to my having allowed

there to be a tax rate on capital τk and defining r2 = (1 − τk )r.
...
government by solving backwards. Starting with the second period:
   The consumer chooses c2 and n2 to maximize second-period utility:
                          ...
                          ln c2 + α ln(n̄ − n2 ) + β ln g2                 (3.20)

subject to the second period budget constraint:

                              c2 ≤ k2 r2 + n2 a(1 − τ2 )                   (3.21)
...
lump-sum tax. It is more generally termed a capital levy.
    However, if we now go back to the first period, consumers will clearly wish
...
by working one’s way up the game tree.
...
issues.
     5 Except in totalitarian societies.
...
curve:
                            y = ȳ + b(π − π e )                     (3.24)
    To think about policy, we will need to specify society’s preferences over
inflation and output. We will not derive this explicitly from utility, but simply
make the assumption that society has the following quadratic loss function:
                              1             1
                         L=     (y − kȳ)2 + a(π − π ∗ )2 ,                  (3.25)
                              2             2
where k > 1. Thus, society has some optimal level of inflation π ∗ , which one
...
desires a level of output which is above the natural rate ȳ. This assumption
...
People believe the level of inflation they precommit to, so that π = π e . Given
this, y = ȳ. Thus the loss-minimizing level of π to commit to is π ∗ .
    Second, assume that the government can’t credibly commit to a level of
...
π e as given.
    Taking the first-order condition for the above loss function implies:
                   ...
                   π=          πe +        (k − 1)ȳ +        π∗             (3.27)
                        a + b2      a + b2             a + b2
    In a rational expectations equilibrium, π = π e , i.e. actual and expected
...
74                                  CHAPTER 3. MACROECONOMIC POLICY
     ...
     2. Impose a punishment equilibrium. In a repeated game, have people expect
        the optimal level of inflation π ∗ as long as the central bank cooperates,
     ...
     4. Delegation
        Suppose the loss function for society is:
                                ...
                                1             1
                              L=  (y − kȳ)2 + a1 (π − π ∗ )2                     (3.29)
                                2             2
        Now suppose the central banker has the loss function

                               1              1
                              L= (y − kȳ)2 + a2 (π − π ∗ )2                      (3.30)
                               2              2
        and we have the same Phillips curve as before.
        Under the no precommitment solution, our solution to the level of inflation
        now becomes:

                                                 b
                                     π = π∗ +      (k − 1)ȳ                      (3.31)
                                                a2
        whereas the solution if the central banker has society’s preferences would
        be:

                                                 b
                                     π = π∗ +      (k − 1)ȳ                      (3.32)
                                                a1
        Provided a2 > a1 , inflation will be lower when authority is delegated to a
        central banker with preferences different from society’s preferences. This
        model in fact provides a rationale for a conservative central banker; that
        is, a person who dislikes inflation more than society does.
        It might initially seem that we would want to have a central banker with
        the largest possibly dislike of inflation; that is, to let a2 go to infinity. But
...
3.2.3     Reputation
If there is uncertainty about the policymaker’s preferences, even a policymaker
whose loss function places a very high weight on inflation may have difficulty
...
may later be useful to them.
    We will see this first in a two-period model which follows Romer’s treatment,
...
which was the key in the original model.
   This equals, after substituting our usual Phillips curve,
                           ...
                           b(πt − πte ) − aπt2                        (3.34)
                                         2
  Assume the discount rate for society is β, so that the goal is to maximize
W = W1 + βW2 .
  Now assume there are two kinds of central bankers:

   • Type 1, who follow society’s preferences
   • Type 2, who have an infinite a and will always set π = 0.
...
than π = 0, if the public ever observes there is a nonzero inflation rate in period
...
between:
   ...
   • Playing a zero inflation rate, which may increase the public’s posterior
...
a Type 1.
   Working backwards, whatever the level of π e , the Type 1 central banker’s
problem is to maximize welfare given π2e . This will lead to a value for π = ab . He
76                                     CHAPTER 3. MACROECONOMIC POLICY

will inflate, since he or she has nothing to lose. In the first-period, if he chooses
π1 6= 0, he will be revealed to be of Type 1. He will then treat the first-period
problem as a one-period problem, since the degree to which he inflates doesn’t
affect next period’s expected inflation. Thus, he will set π1 = ab .
    Given this, the value of his objective function for the two periods is:

                                 b         1 b         1 b
                   WIN F = [b( − π1e ) − a( )2 ] − β a( )2                  (3.35)
                                 a         2 a         2 a
    The other possibility is to choose π = 0. The two possible classes of solution
for this game-theoretic model would be for him to play either π1 = 0 or π1 =
 b
...
randomization case is the more general one, so we will examine that one.
    Let q be the probability that the Type 1 central banker sets π1 = 0. Suppose
π1 = 0 is observed. Then the public knows that it either has a Type 2 central
...
probability by Bayes’ Rule:
                                         ...
                                         P (π = 0|T ype1)P (T ype1)
     P (T ype1|π = 0) =
                           P (π = 0|T ype1)P (T ype1) + P (π = 0|T ype2)P (T ype2)
                                                                               (3.36)
     which implies:
                                                          qp
                             P (T ype1|π = 0) =                                           (3.37)
    ...
    Expected inflation in period two is then π2e = P (T ype1|π = 0) ab + (1 −
P (T ype1|π = 0))0, or
...
banker randomizes and in fact chooses π = 0 as:

                                         b     pq     b    1 b
                 W0 (q) = b(−π1e ) + β[b( −             ) − a( )2 ]                       (3.39)
                                         a 1 − p + pq a    2 a
     This simplifies to:
...
plays π = 0 even when Type 1, he gets no reputation effect from playing π = 0.
   6 Note that if he played π = 0 with certainty, so that q = 1 the public’s posterior probability
                             1
of his being Type 1 would be equal to p. Thus there would be no reputational effect
3.3. THE LUCAS CRITIQUE                                                        77

    As q gets very small, observing π1 = 0 strongly suggests to the people that
the central banker is type 2. Thus, type 1’s benefit from a big reputation effect.
Suppose that W0 (0) < WIN F . Then it is never worthwhile for the type 1 central
banker to play π1 = 0, because the second-period benefits he will get from the
reputation effect aren’t enough to offset the first-period losses. One can show
that this will occur if β < 21 . Thus, if the central banker doesn’t care very
much about the future, one reverts to the one-shot game, with the usual time
consistency problems.
    Now suppose that W0 (1) > WIN F . Then, even though the central banker
doesn’t in fact increase the probability that he is Type 2, it will be worthwhile
for him to pretend that he is Type 2 just to avoid people raising p = 1. This
will occur if β > 21 1−p
                      1
                         , which is greater than 12 .
    Finally, suppose that W0 (0) > WIN F > W0 (1). Then there is an incentive
to randomize. Clearly from above this situation will occur if the discount rate
β is between 12 and 12 1−p1
                            . The actual q chosen will be just at the point where
the policymaker is indifferent between inflating and not inflating. One can show
that this implies that q = 1−p p (2β − 1) (and that over the range of permissible
β, q therefore ranges continuously between 0 and 1.
    As stated, this may seem more like a model of deception rather than repu-
tation. The Type 1 policy makers can, by playing low inflation, fool the public
in believing they are Type 2, and then exploit this effect in the second period.
But note that the Type 2 policymakers are also benefitting in a way. They will
always want to play π1 = 0, and will receive the benefit of the reputation effect.
...
between playing π = 0 or inflating. If he plays π = 0, he gains in reputation,
and expected inflation is lower. If he inflates, he gets a one-period gain, but
...
thus the incentive to inflate increases. Thus, as before, we may have equilibria
where Type 1’s never inflate, ones where they always inflate, and now new ones
where they inflate after some period of time.
    Also note that the discount rate is key here. If the discount rate is too low,
it isn’t worthwhile for Type 1’s to build up any form of inflation. One could
recast this model into a model in which people are uncertain about the Central
bank’s discount rate, to obtain a similar sort of result.
...
curve.
    Suppose that the following AS and AD equations describe the economy:
                              ...
                              yt = α(pt − Et−1 pt )                       (3.41)
                                     yt = mt − pt                         (3.42)

     Suppose that the Fed’s monetary policy is initially

                               mt = µ1 + mt−1 + ²t                        (3.43)
   where ²t is an iid shock reflecting the Fed’s inability to completely control
the money supply.
   By substitution, we can easily show that:
                                    α
                            yt =       (mt − Et−1 mt )                    (3.44)
                                   1+α
    Using the stochastic process above, we can show that mt − Et−1 mt = (mt −
mt−1 )−µ1 , and hence that the relationship between output and observed money
is:
                               α                  α
                       yt =       (mt − mt−1 ) −     µ1                   (3.45)
                              1+α                1+α
    In other words, the following graph (Figure 3.2).
    Suppose the Fed doesn’t know that the economy is described by the Lucas
...
attempts to exploit this, by for example setting mt = µ2 + mt−1 + ²t , where
µ2 > µ1 , what will happen is that the relationship between money and output
will change to:
                             α                     α
                       yt =      (mt − mt−1 ) −        µ2                 (3.46)
                           1+α                    1+α
   In other words, the intercept will shift, and you won’t in fact get any more
output (Figure 3.3).
...
by central banks aren’t likely to be useful. The above example was deliberately
simple, to illustrate the point. In practice, it is often difficult after estimat-
...
80                                 CHAPTER 3. MACROECONOMIC POLICY
                          ...
                          µ ˙ ¶
                           ...
                           Y             Y     Y
                                                                       ¡ D˙ ¢
     where γ = ẎY . Hence a constant debt-GDP ratio implies that       Y       = 0, or:

                                        G − T − Ṁ
                                                P
                                  D=                                               (3.49)
                                          γ−r
    Now if γ −r > 0, we can have a constant debt-GDP ratio even if expenditures
...
pay off the debt. Thus, tighter money now may lead to looser money later,
...
3.5        Problems
     1. Suppose there are two political parties, labeled D and R. Party D has a
        utility function over inflation of − a2D π 2 + bD (π − π e ) and party R has a
        utility function over inflation of
        − a2R π 2 + bR (π − π e ), where bR < bD and aR > aD . The economy is
        described by a standard ‘expectations-augmented’ Phillips curve, y = ȳ +
        β(π − π e ).
3.5. PROBLEMS                                                               81
    ...
    (b) Suppose there is an election next period. Party D has probability
        p of winning. What is expected inflation? Compare the behavior of
        output and inflation assuming party D wins against when party R
        wins.
    ...
    (d) Compare the solution obtained when the government can commit for
        the case where φ = 1 to the case where φ = 0. In which case is
        inflation bigger? Why?
     (e) Now assume that there are two periods and two types of policy mak-
         ers. Type 1 policymakers have the loss function Lt = πt2 , and type
         ...
         discount rate is β, so that L = L1 + βL2 . For simplicity, assume that
...
82                                CHAPTER 3. MACROECONOMIC POLICY
       ...
       about ATM machines). You believe prices are inflexible in the short run.
     ...
     3. As you know, the European Monetary Union (EMU) has recently adopted
        ...
        (a) Suppose a country has a different target level of output than the
        ...
        (b) Now suppose that a country has a different weight on inflation in
         ...
         (c) Now suppose that a country has the same loss function as the ECB,
             but has a different slope to its short-run aggregate supply curve.
...
3.5. PROBLEMS                                                              83
 ...
 5. Consider the following version of the IS-LM model modified to incorporate
    ...
    the real interest rate is constant at a level r = r̄. Also, assume that
    (1 − c)r̄ > αθ.
    ...
    Now let’s use this to analyze the impact of some currently proposed eco-
    nomic policies.
...
84                                  CHAPTER 3. MACROECONOMIC POLICY
           ...
           (c) Now suppose instead that, because the size of the budget surplus is
               ...
               the effects on output and the shadow price of capital of an expected
     ...
     6. Consider the following economy (due to Weitzman). There are N con-
       ...
       Labor is supplied inelastically to each firm at level N1 L∗.
       ...
       how real wages W P are determined.
           ...
           (b) Assume that the nominal wage W is fixed at some level W̄ . Solve for
       ...
       where ω and λ are parameters and Πi denotes firm i’s real profits.
...
3.5. PROBLEMS                                                               85
     ...
     (e) Solve for the equilibrium levels of output, prices and unemployment.
         ...
         special case that ω = W̄ . For which values of ω and λ, if any, would
 ...
 8. Consider the following simple model of the economy:
                      ...
                      mt − pt = −α(rt + Et πt+1 ) + βyt                   (3.5)
                                         yt = cyt − drt                   (3.6)
                                      πt = πt−1 + θyt ,                   (3.7)

    where all variables are in logs and πt = pt − pt−1 .

     (a) Provide economic interpretations for these equations
    (b) Is anticipated money neutral? Is unanticipated money?
     (c) Write down, but do not solve, a difference equation for pt .
    (d) Solve for pt assuming that mt = m̄, a constant, for all past, present
        and future values of t.
     (e) Solve for pt assuming that mt = µ + mt−1 . Denote the value of m0
         as m̄.
     (f) Would you answer to either of the previous two parts have changed if
         people had backwards-looking expectations, so that Et pt+1 = pt−1 ?
         Why or why not?
...
isn’t the case- saving is only transformed into investment after passing through
financial institutions and markets (e.g. banks, the stock (or equity) market, the
bond market). We’ve also assumed that investment can change instantaneously
...
fluctuations.
    In this section of the course, we’re going to look at several partial equilibrium
models which relax these assumptions.
    First, we’ll consider how investment would be determined by firms in the
absence of adjustment costs and without specifying the source of funds.
    Then, we’ll look at a model in which there are (convex) costs to adjusting
the capital stock.
...
vestment, and look at the effects of uncertainty on both the timing of and the
magnitude of investment.


4.1      The Classical Approach
The simplest theory of investment comes from the familiar profit-maximization
theory from micro, which we’ve already seen and used:
...
88                                                  CHAPTER 4. INVESTMENT
                            ...
                            maxP F (K, L) − RK − W L                             (4.1)
                                                                    R
     ¿From this, we get the usual condition that FK (K, L) = P        , or the capital
...
three components (Hall and Jorgenson (1967)):
     ...
     • The capital depreciates at rate δ

     • By using the capital, the firm forgoes the choice of selling it and investing
       the money at interest rate r

     • The firm may suffer a capital loss or get a capital gain (i.e. the price of
...
we implicitly assumed that pK was always one).
    There are some theoretical problems with this approach; it isn’t forward
...
problem is that it doesn’t seem to fit the data very well. For example, if one
...
4.2       Adjustment Costs and Investment: q The-
          ory
Let’s denote profits as a function of the capital stock as Π(K) (note that Π
is not inflation), where Π0 > 0, Π00 < 0. Suppose that there is now a cost to
...
can also get our answer more intuitively, but let’s first derive it mathematically.
   First, let’s set up the Hamiltonian, in present value terms:

                        H = e−rt [Π(K) − C(I) + q(I − δK)]                              (4.7)
                                                     1
   Here, I’ve called the Lagrange multiplier q, for reasons that will become
apparent later on.
...
would reduce to Π0 (K) = r + δ, which would uniquely define the capital stock,
...
ment can be written as a function of the multiplier q, and that this first-order
condition conveniently gives us an expression for the behavior of q. Let’s see
...
is the same expression derived initially in the user cost of capital case. What’s
different now? Well, in this model we’re going to be able to determine how the
...
q À 1, or if the market value exceeds the replacement cost, it will be optimal
...
to as marginal q. The q in Tobin’s model (which was developed well before the
...
4.2. ADJUSTMENT COSTS AND INVESTMENT: Q THEORY                                 91
                        ...
                        I
form: C(·) = I(1 + φ( K   )). Hence this model preserves Tobin’s insight, and can
actually be tested on the data.
    Note that even if we write q in this present-value way, and specify investment
as a function of it, we haven’t entirely determined the level of investment, since
q depends on the future value of the capital stock, which in turn depends on the
current level of investment. To fully analyze this model, we’re going to have to
turn to the same phase-diagram analysis we used before. This time, we’ll use
K and q as our two variables.
    The two equations of motion, again, are:
                              ...
                              q̇ = (r + δ)q − Π0 (K)                       (4.13)
                                 ...
                                 K̇ = I(q) − δK                            (4.14)
    Let’s derive the usual phase diagram (Figure 4.1). Note that the K̇ = 0 locus
slopes upwards and the q̇ = 0 locus slopes downwards. Like most other economic
...
investment):
     ...
     which implies:
                                 ...
                                 Ḣ
                                    = π(p) − δh.                          (4.22)
                                 N
   Let’s think for a second about what the steady state in this model should
...
time. So we don’t want to try to have a steady state where Ḣ = 0. Instead,
we will use the trick from growth theory, and look for a steady state where per
capita housing stock is zero, that is, ḣ = 0.
...
4.3. CREDIT RATIONING                                                           93
                              ...
                              ḣ = π(p) − (n + δ)h.                         (4.23)
   So we have ḣ and ṗ equations, where p is playing the role q did before.
   We first draw the loci: the ḣ = 0 locus clearly slopes up and the ṗ = 0 locus,
   ṗ = (r + δ)p − R(h) (where [R0 < 0]), clearly slopes down (Figure 4.2).
   Using the model:

  1. What happens to the steady state value of p if the rate of population
     growth (n) increases?
      Answer: the ḣ = 0 locus shifts up, while the ṗ = 0 locus is unchanged. So
  ...
  2. What is the effect of an immediate, permanent increase in the rate of
     population growth?
  ...
  3. What is the effect of an increase in the rate of population growth that is
...
first consider a model due to Stiglitz and Weiss (AER, 1981)in which adverse
...
to the pdf f (R, θ), where θ indexes increasing risk in terms of a mean-preserving
spread.
...
If the interest rate is r̂, the borrower will be in default if C + R ≤ B(1 + r̂). If
...
to the project there is.
    The profit the entrepreneur makes on the project is then equal to
                             ...
                             π = max(R − (1 + r̂)B, −C)                              (4.24)

     The return to the bank is then

                              ρ = min(R + C, B(1 + r̂)).                             (4.25)
...
risk)2 This is the key issue in the analysis which follows.
    The preceding point immediately implies that for a given r̂, there is some θ̂
such that entrepreneurs only borrow if their θ > θ̂. Furthermore, ddr̂θ̂ > 0.
    The increase in the interest rate means that, ceteris paribus, expected profits
will be lower. Some people who had marginally positive expected profits will
now have negative expected profits, and will not want to invest, raising θ̂.
    An increase in θ̂ will lower the expected return to the bank, however. The
...
thing like the following: (Figure 4.5). The adverse selection effect as depicted
...
return. Let’s think about loan supply and demand. We may determine loan
demand by simply multiplying B by the fraction of entrepreneurs who want
to invest- i.e. those with θ > θ̂. Since θ̂ is increasing in r̂, loan demand is a
downward-sloping function of r̂. To determine loan supply, we need to think
of the bank’s decision of how many loans to make. Assume that the supply of
loans is some positive function of the return to the bank. Then loan supply as
a function of r̂ is also an inverted U-shaped function.
...
haven’t written this into its return function for simplicity.
4.4. INVESTMENT AND FINANCIAL MARKETS                                                    95
...
[Not covered Spring 2002]
    Financial markets exist in part because firms do not necessarily have all the
    funds internally they need to undertake investment projects, and in part
because firms cannot directly borrow from individuals due to asymmetric infor-
mation problems.
    The field of corporate finance has grown up to explain the functioning of
such markets, and in particular in the past twenty years there has been renewed
interest at the theoretical level into why financial markets exist and how they
affect investment decisions. We will only see a very brief introduction into this
field in this part of the course. (Much of this is taken from the Bernanke, Gertler
and Gilchrist paper).
    The key result from that field for this part of the course is that external forms
of finance are more costly than internal ones. This is because people outside
the firm have less information about the probability of success of projects the
firm will undertake than do people inside the firm.
    Let’s use the following model to show this.
    Suppose there are two types of firms, 1 (bad) and 2 (good), distinguished
by their production. A fraction q are type 1, and 1 − q are type 2. Firms know
which type they are, but can’t prove it to outsiders.
    A type i firm can produce zi units of output at no cost (that is, with no
further investment, where z2 > z1 . Alternately, it can invest F , a fixed amount
for either type of firm. Investing F means building a new factory.
    Firms which invest will have an output of either x or 0. θi the probability
of a type i firm having output x.
    Finally, we assume that it would be profitable for each firm to invest. That
is,

                                 θi x − F > zi , i = 1, 2                            (4.26)
    So why won’t both firms invest? The problem is that firms do not have
...
source. The outside source does not know what type of firm each one is.
   3 Why don’t new banks enter and push the interest rate up to the point of intersection?
...
that lenders cannot distinguish between the two types of firms, because presum-
ably if they could they would want to charge different rates. Let’s see whether
...
even though it’s below average. For the good firm, the opposite is true- it pays
...
4.4. INVESTMENT AND FINANCIAL MARKETS                                          97
    ...
    If the inequality isn’t satisfied, then our original assumption that there was
an equilibrium in which both types of firms borrowed at the same interest rate
is violated. It should be clear that we can’t have an equilibrium in which both
firms borrow at different interest rates- the firms which borrowed at the higher
rates would want to switch to the lower rates. Furthermore, we can’t have an
...
borrow. This will be called the separating equilibrium. Let’s verify that this
equilibrium exists.
    The zero-profit condition for lenders is:

                         (F − C) = θ1 (F − C)(1 + RS )                     (4.33)
   which implies that:
...
mixture of good and bad firms.
   The condition for each firm to want to invest is now:
                           ...
                           θ1 [x −           ] ≥ z1 + C                    (4.35)
                                        θ1
   Type 2 firms didn’t want to invest at the pooling equilibrium, and so they
won’t want to invest here, either. For type 1 firms, this inequality reduces to:

                                     θ1 x − F ≥ z1                         (4.36)
  which is true by assumption.
  Hence the separating equilibrium does exist.
  This model is quite similar to Akerlof’s “lemons” model. The idea of that
model is that people can sell used cars for two reasons:

   • The car is fine, but they want a new one for some exogenous reason (they’re
     moving overseas).
...
This is a very old idea (Gresham’s law, that “bad money drives out good” is an
...
Let’s now come to the principal reason for deriving this model, which is to look
at the effect of changing cashflow on investment. First, let’s note that if there
were no information problem, both firms would be charged it’s own interest
...
will decline, and both firms may invest. Hence investment will be higher.
    Now you might object that this story isn’t very realistic; in particular, the
...
the loan is seen as an indication of one’s own probability of success (it also gets
...
to pay off the loan, but that’s another story).
...
value of the firm is unaffected by the debt-equity mix.
    Suppose the firm is financing a project. Let X̃ be the cash flow from the
...
matter.
    ‘Proof’: The firm’s net cash flow (after paying bond holders) = X̃ − (1 + i)B
...
from the firm) and also an amount of debt B. Then her total return would be
                             ...
                             X̃ − (1 + i)B + (1 + i)B = X̃                             (4.37)

    Note that the investor in this case is ”un-leveraging” the firm. Since the
investor is getting X̃ for her investment, we know the price of the investment
must be V (by arbitrage – we defined V as the market price of return X̃). Thus
...
utility of consumption in the states of the world where X̃ is high, on the shape of the distri-
bution of X̃, etc. The beauty of the MM theorem is that we are not going to have to know
how V is determined.
...
[Not covered Spring 2002]
    Banks are a particularly important type of financial institution. Most con-
sumer borrowing is done at banks (particularly mortgages), as is borrowing by
small firms which do not have ready access to equity or debt markets. Banks
...
with probability πi , i = 1, 2 they will need to consume at date i and no other
   5 Bank runs are major plot points in the movies “Mary Poppins” and “It’s a Wonderful
...
(patient) agents is ρu(C2 ). Ex ante, expected utility is:

                            U ≡ π1 u(C1 ) + ρπ2 u(C2 )                      (4.38)
...
fractional reserve banking system.
  ...
  1. Autarkic solution:
      ...
      ¿From the constraints, it is clear that C1 ≤ 1 and C2 ≤ R. This is because
      the investment decision I will always be ex post inefficient, because one
      would have liked to invest nothing if one were a type 1 consumer, and
      everything if type 2. This problem will be fixed if there is a financial
      market.
  ...
  2. Financial Market
      ...
      his entire investment in the illiquid asset). The second has bought 1−I   p
...
102                                                    CHAPTER 4. INVESTMENT
  ...
  3. Pareto Optimal allocation
      ...
      It is unlikely that this will be satisfied by the market allocation. If we
      assume concave utility, it is likely that u0 (1) > ρRu0 (R), so that the
      optimal C1 > 1. This is to ensure consumers against the shock that they
      must consume at time 1.

  4. Banking system
      Now suppose there is an institution which can write contracts on the
      amount (C1 , C2 ) that can be withdrawn at dates 1 and 2 for a deposit
      ...
      will be able to fulfill its contract
      Think of a type 2 consumer who believes that the bank can fulfill his
      contract. This consumer can withdraw the optimal C2 in period 2, or
      withdraw C1 in period 1 and store it until period 2.
      ¿From the first-order condition for the social optimum, if ρR < 1 there
      will be no way to implement the optimal contract. If ρR ≥ 1, two cases
      may occur. If the consumer trusts the bank, she will withdraw in period 2.
      By the Law of Large Numbers, a fraction π1 of consumers will withdraw
      in period 1, which means that the bank must have π1 C1 on hand. This is
      feasible, so the patient consumer’s expectations will be fulfilled.
      ...
      asset value of π1 C1 + (1 − π1 C1 )L, which is less than C1 , implying the
      bank will fail.
4.6. INVESTMENT UNDER UNCERTAINTY AND IRREVERSIBLE INVESTMENT103
...
fact that it is possible the bank will not have enough assets to be able to offset
its liabilities.
...
fail’; it has been argued that this was a leading cause in the recent financial
...
is a sunk cost. We will then add uncertainty to this model, and finally look at
the effects of uncertainty on investment separately. We shall follow models by
Dixit and Pindyck (JEL 1991) and Abel (AER 1983). It will turn out that it is
easiest to analyze such models using stochastic-calculus techniques first applied
to finance theory. These techniques are briefly summarized in Appendix B.
...
rate α, given by V = V0 eαt . For simplicity, let’s use the normalization V0 = 1.
The firm has discount rate µ. Assume µ > α. The firm’s problem, as of time
zero, is to decide when to make the investment.
    It might initially seem that the firm should simply wait until the instant that
...
This solution (which I will call the ‘neoclassical’ solution) is flawed because it
...
recoverable and the firm no longer has the choice of whether or not to invest.
Hence the firm should wait until V exceeds I by the option value.
    The option value is so called because evaluating the firm’s problem is directly
comparable to pricing a financial instrument known as an option. An option
...
T at which the option to invest is exercised.
   ...
   Substituting the expression for V (t), this becomes:
                                ...
                                (e(α−µ)T − Ie−µT )                            (4.47)
    Solving the first-order condition with respect to t implies an optimum value
for T of:
                                         µ      ¶
                                    1       Iµ
                               T = ln              ,                      (4.48)
...
option value.
    Since by assumption µ > α, the value of the project is greater than I. Thus,
...
expression implies that changes in lnV are normal with drift α.
...
value of the option as:
                               max
                       F (V ) = T Et [(VT − I)e−µ(T −t) ]                  (4.51)
   where µ is the discount rate.
...
time, the holder of the asset has two options:
  ...
  1. Sell the asset, receive F , and invest it in the alternative asset from which
     ...
     to him or her will then be µF dt.
...
Since F is a function of a Wiener process, we must use Itô’s lemma to solve for
the derivative. We can write:
   ...
   Substituting for dV , one obtains:
                                            ...
                                            1
                  dF = αV FV dt + σV FV dW + σ 2 V 2 FV V dt               (4.54)
                                            2
    Now Et dZ = 0, since increments of a Wiener process are standard normal.
Thus, Et dF = (αV FV + 12 σ 2 V 2 FV V )dt. We may then rewrite the asset-pricing
relationship as:

                                      1
                          µF = αV FV + σ 2 V 2 FV V                        (4.55)
                                      2
   This is a differential equation in F . We need boundary conditions for this
...
106                                                   CHAPTER 4. INVESTMENT
    ...
    3. FV (V ∗ ) = 1. This is known as the smooth pasting condition. If F (V )
       ...
       at a different point- the reason is that by Jensen’s inequality, the expected
...
following quadratic equations in b1 and b2 :
                          ...
                          1 2 2        1
                            σ b1 + (α − σ 2 )b1 − µ = 0                      (4.57)
                          2            2
                          1 2 2        1
                            σ b2 + (α − σ 2 )b2 − µ = 0                      (4.58)
                          2            2
.
...
the option. One can also show that V ∗ and F (V ) are increasing in σ. In
...
under certainty. Unfortunately, it is difficult to do this analytically, so we will
consider a numerical example. Let µ = .05, α = .03 and σ = .01.6
...
section shows that the effects of uncertainty on investment need not always go
this way.
...
Here, we follow a paper due to Abel (AER, 1983).
   He looks at the effects of uncertainty in the price of output on investment.
                                                                       1−α
   Suppose a firm has a Cobb-Douglas production function Yt = Lα    t Kt   . The
output price is pt , which follows the following geometric Brownian motion:

                                        dpt
                                            = σdW                                      (4.62)
                                        pt
   where dW is again the increment to a Wiener process. The wage is w,
and investment has the convex cost function γItβ , where β > 1. The capital
accumulation equation is dKdt = It − δKt .
                             t
...
therefore be written as:
                               ...
                               Z ∞
        V (Kt , pt ) = maxEt         [ps Lα  1−α
                                          s Ks   − wLs − γIsβ ]e−r(s−t) ds             (4.63)
                                 t

    subject to the laws of motion of K and p.
    This maximization problem has the following Bellman equation:

          rV (Kt , pt )dt = max(pt Lα  1−α
                                    t Kt   − wLt − γItβ )dt + Et dV                    (4.64)
...
that α must be less than µ. σ is comparable to a standard deviation for the growth rate of V .
A value of .01 implies that 95% of the time, the growth rate of V is between 3 and 7 percent.
108                                                CHAPTER 4. INVESTMENT

    To solve the maximization problem on the right-hand-side, we first need to
solve for Et dV
             dt . By Itô’s Lemma,
...
right-hand side go away, leaving us with:
                                               1
                    Et (dV ) = [(It − δKt )VK + pt σ 2 Vpp ]dt             (4.66)
                                               2
   Substituting yields:

                                                       1
      rV = maxpt Lα  1−α
                  t Kt   − wLt − γItβ + (It − δKt )VK + p2t σ 2 Vpp        (4.67)
                                                       2
...
some tedious algebra, one can show that the first two terms reduce to:
        1                           α
                                 α 1−α
    hpt1−α Kt , where h = 1 − α( w )
    (given CRS, this is just capital’s share in output).
    The first order conditions with respect to It imply γβItβ−1 = VK . Substi-
tuting, we get that the resulting Bellman equation is:
                             1                               1
            rV (K, p) = hp 1−α K + (β − 1)γI β − δKVK + pσ 2 Vpp            (4.68)
                                                             2
...
4.6. INVESTMENT UNDER UNCERTAINTY AND IRREVERSIBLE INVESTMENT109
...
Jensen’s inequality. There is a larger chance of reaping much larger benefits,
which outweighs the larger chance of reaping somewhat smaller benefits.
  ...
  1. Write down the optimal control problem:
                                           ...
                                           Z ∞
                       J(x0 ) = max E0           e−ρt f (xt , ut )dt           (4.72)
                                            0
                                            dX = αdt + βdW,                    (4.73)
  ...
  2. Write down the associated Bellman equation:
                       ...
                       ρJ(xt )dt = max[f (xt , ut )dt + E(dJ(xt ))]            (4.74)
  ...
  4. Take expectations to get EdJ
                                                   ...
                                                   1
                          EdJ(xt ) = αJ 0 (xt )dt + β 2 J 00 (xt )             (4.76)
                                                   2
  5. Substitute EdJ into the Bellman equation

                                                            1
                   ρJ(xt ) = max[f (xt , ut ) + αJ 0 (xt ) + β 2 J 00 (xt )]   (4.77)
                                                            2
...
4.7     Problems:
  1. Consider the q theory model of investment presented in class. Add to the
     model taxes on corporate profits at rate τ , so that after-tax profits for a
     capital stock K are (1 − τ )Π(K). Suppose that the economy starts off at
  ...
  2. Consider the following model of the housing market presented in class:
      ...
      the interest rate, n is the rate of population growth, δ is the depreciation
      rate, R() gives rent as a function of the stock of housing per capita, and
      π() gives housing investment per capita as a function of price. R0 < 0,
      π 0 > 0.
      Consider two versions of the model: the usual forward looking one, and
      one in which price expectations are static: E(ṗ) = 0.
  ...
  4. Consider a model of the housing market like the one presented in class,
     ...
     E(ṗ) = k. Analyze the equilibrium in the market. How do the steady
     state levels of h, p, and rent compare to the regular model. Show how
     the effect of a permanent increase in the rate of population growth differs
...
4.7. PROBLEMS:                                                              111
  ...
  5. (old core exam question) Consider the following model of the housing
    ...
    π 0 > 0 (differential equation for housing stock, where n is population
    growth and is the rate of depreciation.)
    Consider the effects in this model of a tax on capital gains (and a corre-
    ...
    effect on the steady state and the transitional dynamics of a permanent,
...
112                                              CHAPTER 4. INVESTMENT
      ...
      seller’s aura whether she is lying).
  ...
  7. Consider a firm which has profits as a function of its capital stock as:
     Π(K). Suppose there is a cost to investment of the form C(I) = b|I|.
     Depreciation occurs at rate δ, and the discount rate is r.
      ...
      (b) Suppose the government suddenly decides to reward investment by
          paying firms τ for each unit of investment they do (where τ < b).
          Assuming we are initially in the steady state, show the path of in-
          vestment, profits and the capital stock over time.
  ...
  8. Suppose a firm is considering investing in a project. The project will yield
     a flow of profits π per unit time, where the behavior of π is described by
     the following:
      ...
      which is not recoverable. The discount rate is ρ.
      For simplicity, let’s first assume that there is no uncertainty, so σ = 0.
      ...
      (b) Solve for the optimal time T ∗ for the firm to invest in the project,
          and the value of the project at that time T ∗ . You may get different
          answers for different assumptions about the parameters; if so, specify
          ...
          recoverable upon disinvestment.
           ...
           Now assume that σ > 0.
       (c) Write down, but do not solve, the differential equation for the value
           of the firm’s option to invest in the project.
4.7. PROBLEMS:                                                                113
    ...
    Suppose a firm has profits as a function of its capital stock of π(K). The
    cost of investing is C(I). Depreciation is linear at rate δ, and the rate of
     ...
     (a) The economy is initially in equilibrium. Suppose it is announced that
         ...
         at rate τ .
          ...
          iii. How would the path of q , I and K differ if instead it had been
               ...
               probability p
     (b) Suppose that due to Global Warming, the rate of depreciation δ
         increases. Plot the behavior of q, I and K from now until the new
         steady state.

  9. Suppose a firm’s profits as a function of its capital stock may be denoted
     Π(K), where Π0 > 0 and Π00 < 0. The firm faces a cost of investment of
     C(I) = I + d(I), where d0 (I) > 0 and d00 (I) > 0. Depreciation occurs at
     rate δ, and the discount rate is r.

     (a) Set up and solve the firm’s maximization problem, and draw the
     ...
     (b) Suppose we are initially in the steady state of the model. Suddenly,
      ...
      (c) Now forget about the asteroid for a moment. Suppose we are in the
          ...
          τ on investment. The tax is expected to be permanent. Show on a
...
114                                              CHAPTER 4. INVESTMENT
 ...
 10. Suppose you own an oil well. This well produces a constant flow x of
     ...
     Brownian motion: dp pt = µdt + σdWt . The well costs some amount z to
                           t
 ...
 11. Consider the housing market model studied in class. Let H denote the
     ...
     rate n) and δ the depreciation rate. Per capita housing is h, with price
     p. Let π(p) denote the gross housing investment function, and let housing
     demand be given by hD = f (R), where f 0 < 0. The real interest rate is r.
      ...
      is, there is some maximum level of rent, R̄, which can be charged for
...
4.7. PROBLEMS:                                                               115
 ...
 12. Consider the q-theory model of investment described in class.
     ...
     deterministic process: V = α0 +α1 t. The firm discounts at the real interest
     rate r.
 ...
 13. Consider the model of investment presented in class in which there is
     ...
     depreciation occurs at a constant rate δ, and profits as a function of the
     capital stock are given by Π(K).

      (a) Write down the two equations of motion characterizing the solution
          to the firm’s optimization problem.
116                                            CHAPTER 4. INVESTMENT

      (b) Suppose it becomes known at time t that at time t + s, a new gov-
          ernment will seize 25% of the firm’s time t + s capital stock, melt it
      ...
      (c) Now suppose instead that it becomes known at time t that at time
          ...
          the firm’s capital stock, and a 50% chance that it will seize nothing.
      ...
      (d) Let’s think generally about the government for a moment. Suppose
...
multiplier effects and why economic situations may be characterized by multi-
...
5.1     Efficiency wages, or why the real wage is too
        high
...
5.1.1       Solow model
The first model is due to Robert Solow.1 Assume production is given by Y =
AF (e( WP )L), where e denotes effort, which depends positively on the real wage
W
 P . The  next subsection will provide one possible rationale for this, but for now
it’s not implausible to think that people might work harder if paid more.
     The firm’s objective is profit maximization. We will assume that it can
...
at the market-clearing level. The firm’s problem is then:
                           max
                       W                    W       W
                      { }, {L}Π = AF (e( )L) −        L                       (5.1)
                       P                    P       P
...
firm cares about effective labor, e( W
      ...
      W
is e( PW ) . At the point where the elasticity of effort is equal to one, changes in
       P
the real wage don’t change the cost of effective labor
    This level of the real wage can easily be above the market-clearing level.
Moreover, note that productivity disturbances have no effect on the real wage,
...
are more afraid of losing their job if the real wage is high. Let L̄ be the number
of workers and N the number of firms. In discrete time, each worker tries to
           P ³ 1 ´t
maximize       1+r    Ut , where Ut = Wt − et if employed and is zero if unem-
ployed. et denotes effort, and may take two values: ē and 0. Hence, people are
in one of three states:
  1 And supposedly originated as a question on a core exam at MIT.
...
Then the worker’s choice is between been unemployed, or being employed and
...
the real wage may be above the market clearing level.
   ...
   • Insider-Outsider Models: These models are models of union behavior.
     ...
     ing with firms to ensure a high real wage, at the expense of employment
...
with the height of the tree, so you don’t climb up every tree you come across,
...
to climb up higher trees, that won’t increase the number of trading partners,
...
5.2.1     Setup
   ...
   3. Decision
         ...
         • He will set a reservation cost of c∗ , and only take opportunities which
...
122   CHAPTER 5. UNEMPLOYMENT AND COORDINATION FAILURE
        ...
        • Assume that people can’t consume their own production but must
...
Let’s think about the fraction of the population trading at any point in time.
...
economy who can trade. Therefore,
                             ...
                             ė = a(1 − e)G(c∗ ) − eb(e).                   (5.15)
     The steady-state condition is ė = 0.
     One can show that the steady-state level of e is increasing in the value of
c∗ ; that is, the fraction of the population trading in the steady-state increases
when peoples’ reservation cost level increases.
     Next, we need to specify how c∗ is determined. Let We and Wu be the
...
receiving (y − We + Wu ) with probability b.
    One can combine these by noting that
                        ...
                        c = We − Wu =                                       (5.18)
                                      r + b + aG(c∗ )
                                           ∗
   ¿From which one can show that dc
                                 de > 0.
...
the first opportunity which comes along which exceeds (or, in some models, is
...
there is frictional unemployment. One can find this level by working out what
...
5.3     Coordination Failure and Aggregate Demand
        Externalities
Let’s now switch gears and look at another set of models which explore the
implications of failure to coordinate across agents. These models of coordination
failure aren’t a huge departure from the unemployment models, since both imply
...
5.3.1    Model set-up
  1. Consumers
        ...
        • Consumers receive income from the profits of firms and wages. They
          have an endowment of labor L, which they supply inelastically. If we
          normalize the wage rate to one, then total income y = Π + L.
124   CHAPTER 5. UNEMPLOYMENT AND COORDINATION FAILURE
  ...
  2. Firms
       ...
       • There are two types of firms in each sector:
          ...
          (b) A unique firm with access to an increasing returns to scale tech-
              nology. It can pay a fixed cost of F units of labor and then one
              unit of labor produces α > 1 units of output. Workers don’t like
              working in the IRS technology, so they command a factory wage
              premium of v.
       • The choice by the firm whether or not to adopt the technology de-
         pends on whether or not they can earn positive profits by doing so.
  ...
  3. Equilibrium
       ...
       • When n firms industrialize, aggregate profits Π(n) = n(ay − F ).
       • This yields aggregate income y = L−nF
                                          1−na
       • In this case, demand is increased by additional investment only if
                                                dy     π(n)
         that investment is profitable, since dn    = 1−an  . Hence there is no
         coordination failure. If it is profitable to invest, all firms will want
         to invest, which will additionally raise the profit from investing.
  ...
  4. Equilibrium when there is disutility to working.
       ...
       • If no one industrializes, there are no profits, and aggregate demand
         y = L.
       • If everyone industrializes, y = α(L − F )
5.3. COORDINATION FAILURE AND AGGREGATE DEMAND EXTERNALITIES125

        • It is quite possible that profits to individual firms when no one else
          industrializes are negative, but profits are positive when everyone
          industrializes. Hence there is coordination failure- everyone would be
          better off if everyone industrialized, but no one firm has the incentive
          to do so.

    Murphy, Shleifer and Vishny go on to give a dynamic version of their model.
    This model, although highly stylized, essentially confirms Rosenstein-Rodan’s
...
nality. If all sectors industrialize at once, the income generate from the profits
of those sectors will raise aggregate demand enough that it is profitable for each
firm to have industrialized. If no sectors industrialize, no single sector has an
...
further examples in macroeconomics are:
   ...
   • Models with trading externalities (or thick-market externalities): The
     ...
     an incentive to trade. We’ll see a model of this when we look at search.
...
5.3.2    Assumptions
   ...
   • Each player chooses an action ei ∈ [0, E] to maximize some payoff
     function σ(ei , e¬i ; θi ). The ¬i subscript simply means the payoff to agent
     i depends on the actions of all other agents (i.e. those agents which are
     not i). θi is a shift parameter; that is, it is an easy way of parameterizing
     changes in functional form. Instead of having the functioning changing,
     we have θ changing.
                                                            2        2
   • There are appropriate second-order conditions ( ∂∂eσ2 < 0, ∂e∂i ∂θ
                                                                     σ
                                                                        i
                                                                          > 0).
                                                                i
   ...
   • Let’s restrict ourselves to considering symmetric Nash equilibria (SNE).

        – Let V (ei , ē) denote the payoff to i when i chooses ei and everyone
          else chooses ē; i.e. V (ei , ē) = σ(ei , ē, θi ).
126   CHAPTER 5. UNEMPLOYMENT AND COORDINATION FAILURE

        – The SNE is characterized by the set S = {e ∈ [0, E] : V1 (e, e) = 0}.
          I.e. the equilibrium has everyone doing the same action, and no one
          changes his or her payoff by changing his or her action This is simply
          from the first-order condition for maximization.
   ...
   • The social optimum is defined by a Symmetric Competitive Equilibrium
     (SCE), the (symmetric) equilibrium a social planner would pick: S̃ = {e ∈
     [0, E] : V1 (e, e) + V2 (e, e) = 0}.


5.3.3    Definitions
  1. If V2 (ei , ē) > 0, the game exhibits positive spillovers. Increases in other
     players’ actions increase your payoff.

  2. If V12 (ei , ē) > 0 the game exhibits strategic complementarity. This implies
     that increases in other players’ actions lead you to want to increase your
     own action. To see this, totally differentiate the first-order condition for
     the SNE: V11 (ei , e)dei + V12 (ei , e)de = 0, which implies: de            V12
                                                                         de = − V11 .
                                                                           i


     V11 < 0, from concavity of the original payoff function. Hence for this
     derivative to be positive, V12 > 0. We can’t assume that de     de > 0 directly,
                                                                       i
     ...
     5.3).
        ...
        ∂ei     dei     d    ej
  3. If ∂θ i
             <  dθi   <   dθi , the game exhibits multiplier effects. The first term
     measures the initial response of agent i to a shock to his payoff function,
...
Given those definitions, we can establish the following:
  ...
  1. Strategic complementarity is necessary for multiple SNE.
      ...
      plementarity, de        V12
                       de = − V11 and hence is positive and greater than one if
                         i
...
5.4. PROBLEMS                                                                   127
  ...
  4. If there are positive spillovers everywhere, we can Pareto-rank the SNE;
     higher actions are preferred.
      Proof: dV (ede
                  i (e),e)
                           = V1 + V2 = V2 hence all agents payoffs are increasing
...
128   CHAPTER 5. UNEMPLOYMENT AND COORDINATION FAILURE
      ...
      (a) Write down the relationship between the unemployment rate u = UL,
                               V
  ...
  2. Consider the following model of the labor market. Suppose the economy
     ...
     function is Y = S α U β . Let WS denote the nominal wage paid to skilled
     workers,WU the wage paid to unskilled workers and P the price level.
     Let S̄ denote the number of skilled workers and Ū denote the number of
     ...
     inelastically. The price level is perfectly flexible.
      ...
      (b) Now suppose that a minimum wage W̄ is imposed for the unskilled
      ...
      where Û is the number of unskilled workers employed and U I is the (ex-
...
5.4. PROBLEMS                                                                129
 ...
 3. Let’s consider the following model of unemployment through job creation
    ...
    bility θq(θ), the worker finds and accepts job. θ = uv , where v is the ratio
...
Many macroeconomic problems have the form:
                  ...
                  max
                                      Z ∞
                 {u(t)}V (0, x0 ) =         e−rt J(x(t), u(t), t)dt            (23)
                                      0
                                 .3.      ẋ(t) = f (x(t), u(t), t)            (24)
                                                      x(0) = x0 .              (25)
...
the (present-value) Hamiltonian:
                ...
                H = e−rt [J((x(t), u(t), t)) + q(t)f (x(t), u(t), t)] ,        (26)
...
varies over time, and I have occasionally referred to it as such in the text.
   Necessary conditions for optimality are:
                  ...
                  ∂x
                                                    lim
    In addition, the transversality condition t → ∞e−rt q(t)x(t) = 0 is often, but
not always, necessary. Concavity restrictions on J and f are needed to ensure
sufficiency of these conditions.
    In Harl Ryder’s class, you will see a full treatment of both necessary and
sufficient conditions for optimality for these problems, as well as a discussion of
other techniques.
    Foremost among these techniques is Bellman’s principle of optimality. This
states that:
                               max
...
changing.
   An example of the use of the Hamiltonian: optimal growth.
                                        ...
                                        Z ∞
                    max                              C 1−σ − 1
                   {u(t)}V (0, x0 ) =         e−rt             dt          (30)
                                        0              1−σ
                                  .3.       k̇ = f (k) − c − δk            (31)
                                                     k(0) = k0 .           (32)
                       1−σ
    In this case, J = C 1−σ−1 ,f = f (k) − c − δk, x = k and u = c.
    Sufficient conditions for optimality are then:
                 ...
                 ∂x
                                            lim
   and the transversality condition t → ∞e−rt q(t)k(t) = 0.
   By time-differentiating the first condition and inserting it into the second,
...
ments property is implied by efficient markets theories of asset pricing.
   A process Wn (t)is a random walk if it satisfies the following:

   1. Wn (0) = 0

   2. Time intervals are of length n1 , where n is a positive integer.

   3. W can only move up and down by the amount √1n

   4. The probability of moving up=the probability of moving down= 12 .
...
Motion or a Wiener Process.4 A process Wt is a Brownian motion5 if and only
if:

   1. Wt is continuous and W0 = 0
   3 This section is largely drawn from Baxter and Rennie, Financial Calculus.
   4 It is known as the former because it well describes the behavior of a small particle trapped
...
techniques we use.
   First, we must define a stochastic process X − t as follows:
                                   Z t              Z t
                       Xt = X0 +         σs dWs +         µs ds,            (36)
                                    0                0

    where dWs is the increment of a Wiener process.
...
drift term from time zero to time t. If µ were constant, the second term would
simply be µt.
    It is often useful to write this in differential form:

                             dXt = σt dWt + µt dt.                          (37)
   The next question is how to compute differentials of functions of these
stochastic processes; i.e. what is df (Xt )? The answer is given by Itô’s Lemma:
   If dXt = σt dWt + µt dt, then:
                                                      1
              df = (σt f 0 (Xt ))dWt + (µt f 0 (Xt ) + σt2 f 00 (Xt ))dt.  (38)
                                                      2
   Note that the standard non-stochastic version of this would not have the
second term in parentheses, 12 σt2 f 00 (Xt ).
   As an example of how this works, consider the function F (Xt ) = Xt2 for the
process dXt = σdWt + µdt. Then, f 0 (Xt ) = 2Xt and f 00 (Xt ) = 2. Hence

                     df (Xt ) = (2σXt )dWt + (2µXt + σ 2 )dt                (39)
    In most economic problems we deal with in Ec208, we will often be taking
conditional expectations of the differential. The important point to note here
is that because increments in the Brownian motion are independent with zero
mean, Et dWt = 0. Hence the general expression
                                                                                   135

      for df derived above simplifies to:
                                             1
                      Et df = (µt f 0 (Xt ) + σt2 f 00 (Xt ))dt,            (40)
                                             2
...
X1t = σ1t dWt + µ1t dt and X2t = σ2t dWt + µ2t dt,
 ...
 +(µ1t      + µ2t      + σ1t   2  + σ2t    2  + σ1t σ2t           )dt. (41)
