---
normalized_id: shared-pdf-reference-harrison-waldron-mathematical-economics-and-finance
exam_code: SHARED
material_scope: harrison ~ waldron - mathematical economics and finance.pdf
material_group: shared
document_type: pdf-reference
source_repo_path: resources/source-material/shared/Harrison ~ Waldron - Mathematical Economics and Finance.pdf
conversion_method: pdftotext
qa_status: pending
---
# shared-pdf-reference-harrison-waldron-mathematical-economics-and-finance
...
conclusions. Proof by contradiction and proof by contrapositive are also assumed.
                                                                       1
There isna book on proofs by Solow which  o should be referred to here.
<N+ ≡    x ∈ <N : xi ≥ 0, i = 1, . . . , N is used to denote the non-negative or-
                                    n                               o
thant of <N , and <N++ ≡     x ∈ <N : xi > 0, i = 1, . . . , N used to denote the
positive orthant.
>
...
8                                                  1.4. MATRIX ARITHMETIC
    ...
    • The scalar product of two vectors in <n is the matrix product of one written
...
So we have C = AB if and only if cij = k = 1n aik bkj .
                                          P
...
The additive and multiplicative identity matrices are respectively 0 and In ≡ δji .
...
equations, both using elementary row operations.
     ...
     3. Each step above (about six of them depending on how things simplify)
        ...
        trix, say
                           E6 E5 E4 E3 E2 E1 (A b) = (I x) .          (1.4.1)
           Picking out the first 3 columns on each side:
                                          E6 E5 E4 E3 E2 E1 A = I.                            (1.4.2)
           We define
                                         A−1 ≡ E6 E5 E4 E3 E2 E1 .                            (1.4.3)
...
A matrix is symmetric
               −1
                        if it is its own transpose; skewsymmetric if A> = −A.
                               >
          
Note that A>        = (A−1 ) .
Lots of strange things can happen in matrix arithmetic.
We can have AB = 0 even if A 6= 0 and B 6= 0.
...

    ordered
      
            n-tuple (x1 , x2 , . . . , xn ) is identified with the n × 1 column vector
  x1
 x2 
     
 . .
...
The distance between two vectors is just k u − v k.
...
the cosine rule.
              ...
              k v − u k2 = (v − u) . (v − u)                                  (1.5.1)
                         = k v k2 + k u k2 −2v.u                              (1.5.2)
                         = k v k2 + k u k2 −2 k v kk u k cos θ                (1.5.3)
...
Definition 1.6.1 The vectors x1 , x2 , x3 , . . . , xr ∈ <n are linearly independent if
and only if
                             r
                             X
                                   αi xi = 0 ⇒ αi = 0∀i.
...
Definition 1.12.1 An n × n square matrix A is said to be
       positive definite ⇐⇒ x> Ax > 0 ∀x ∈ <n , x 6= 0
 positive semi-definite ⇐⇒ x> Ax ≥ 0 ∀x ∈ <n
      negative definite ⇐⇒ x> Ax < 0 ∀x ∈ <n , x 6= 0
 negative semi-definite ⇐⇒ x> Ax ≤ 0 ∀x ∈ <n
...
A is positive/negative (semi-)definite if and only if P−1 AP is.
...
matrices of random variables. Since
                           ...
                           vij = Cov [r̃i , r̃j ] = Cov [r̃j , r̃i ]                 (1.12.1)
...
and
                       ...
                       >
                                        X
                      w Vw =                      wi wj Cov [r̃i , r̃j ]             (1.12.2)
                                        i=1 j=1
                                                               
                                             XN        N
                                                       X
                                   =    Cov  wi r̃i ,   wj r̃j                     (1.12.3)
                                                i=1         j=1
                                              N
                                              X
                                   = Var[           wi r̃i ] ≥ 0                     (1.12.4)
...
2.3 Vector-valued Functions and Functions of Sev-
    eral Variables
...
set of a vector space, say <N . Such a function has N component functions.
...
y ∗ as x → x∗
 ⇐⇒
∀ > 0, ∃δ > 0 s.t. k x − x∗ k< δ =⇒ |f (x) − y∗ )| < .

This is usually denoted
                                  lim f (x) = y ∗ .
                                 x→x∗

Definition 2.3.10 The function f : X → Y (X ⊆ <n , Y ⊆ <) is continuous at x∗
 ⇐⇒
∀ > 0, ∃δ > 0 s.t. k x − x∗ k< δ =⇒ |f (x) − f (x∗ )| < .
...
Definition 2.3.12   1. The correspondence f : X → Y (X ⊆ <n , Y ⊆ <) is
      upper hemi-continuous (u.h.c.) at x∗
        ⇐⇒
       for every open set N containing the set f (x∗ ), ∃δ > 0 s.t. k x − x∗ k<
       δ =⇒ f (x) ⊆ N.
       (Upper hemi-continuity basically means that the graph of the correspon-
       dence is a closed and connected set.)

     2. The correspondence f : X → Y (X ⊆ <n , Y ⊆ <) is lower hemi-continuous
        (l.h.c.) at x∗
        ⇐⇒
       for every open set N intersecting the set f (x∗ ), ∃δ > 0 s.t. k x − x∗ k<
       δ =⇒ f (x) intersects N.

     3. The correspondence f : X → Y (X ⊆ <n , Y ⊆ <) is continuous (at x∗ )
        ⇐⇒
...
Note that if f : <n → <, then, strictly speaking, the second derivative (Hessian) of
...
Theorem 2.5.1 (The Chain Rule) Let g: <n → <m and f : <m → <p be contin-
uously differentiable functions and let h: <n → <p be defined by
...
Let g: <n → <m and f : <m+n → <p be continuously differentiable functions, let
x ∈ <n , and define h: <n → <p by:

                                    h (x) ≡ f (g (x) , x) .
                                                                  ∂h   i
The univariate Chain Rule can then be used to calculate ∂x          j
                                                                      (x) in terms of partial
derivatives of f and g for i = 1, . . . , p and j = 1, . . . , n:
            m                               m+n
 ∂hi       X   ∂f i             ∂g k        X ∂f i               ∂xk
     (x) =          (g (x) , x)      (x) +           (g (x) , x)     (x) . (2.5.1)
 ∂xj       k=1 ∂xk              ∂xj        k=m+1 ∂xk             ∂xj
...
we can use (2.5.2) to write out the total derivative h0 (x) as a product of partitioned
matrices:
                h0 (x) = Dg f (g (x) , x) g 0 (x) + Dx f (g (x) , x) .          (2.5.4)
Theorem 2.5.2 (Product Rule for Vector Calculus) The multivariate Product Rule
comes in two versions:
     1. Let f, g: <m → <n and define h: <m → < by
                                            h (x) ≡ (f (x))> g (x) .
                                            | {z }       |     {z   } | {z }
                                               1×1            1×n        n×1
     ...
     2. Let f : <m → < and g: <m → <n and define h: <m → <n by
                                               h (x) ≡ f (x) g (x) .
                                               | {z }        | {z } | {z }
                                                n×1           1×1    n×1
...
Theorem 2.6.1 (Implicit Function Theorem) Let g: <n → <m , where m < n.
...
in particular at z∗ . But we know from (2.5.4) that
                            ...
                            f 0 (z) = Dy gh0 (z) + Dz g.
                                                                 ...
                                                                 Revised: December 2, 1998
24                                            2.7. DIRECTIONAL DERIVATIVES


Hence
                            Dy gh0 (z) + Dz g = 0m×(n−m)
and, since the statement of the theorem requires that Dy g is invertible,
...
To conclude this section, consider the following two examples:
     ...
     1. the equation g (x, y) ≡ x2 + y 2 − 1 = 0.
        Note that g 0 (x, y) = (2x 2y).
                           √                    √
        We have h(y) = 1 − y 2 or h(y) = − 1 − y 2 , each of which describes a
                                                                               ∂g
        single-valued, differentiable function on (−1, 1). At (x, y) = (0, 1), ∂x =
     ...
     2. the system of linear equations g (x) ≡ Bx = 0, where B is an m × n
        matrix.
        We have g 0 (x) = B ∀x so the implicit function theorem applies provided
        the equations are linearly independent.


2.7 Directional Derivatives
Definition 2.7.1 Let X be a vector space and x 6= x0 ∈ X. Then

     1. for λ ∈ < and particularly for λ ∈ [0, 1], λx + (1 − λ) x0 is called a convex
        combination of x and x0 .

     2. L = {λx + (1 − λ) x0 : λ ∈ <} is the line from x0 , where λ = 0, to x,
        where λ = 1, in X.

     3. The restriction of the function f : X → < to the line L is the function

                           f |L : < → <: λ 7→ f (λx + (1 − λ) x0 ) .

     4. If f is a differentiable function, then the directional derivative of f at x0 in
        the direction from x0 to x is f |0L (0).
...
CHAPTER 2. VECTOR CALCULUS                                                                    25
       ...
       • Note that, by the Chain Rule,
                              ...
                              f |0L (λ) = f 0 (λx + (1 − λ) x0 ) (x − x0 )                (2.7.1)

         and hence the directional derivative

                                      f |0L (0) = f 0 (x0 ) (x − x0 ) .                   (2.7.2)
...
28                                                   3.2. CONVEXITY AND CONCAVITY
     ...
     1. f is a convex function
           ...
           ∀x 6= x0 ∈ X, λ ∈ (0, 1)
                              ...
                              f (λx + (1 − λ)x0 ) ≤ λf (x) + (1 − λ)f (x0 ).        (3.2.1)
     ...
     2. f is a concave function
           ...
           ∀x 6= x0 ∈ X, λ ∈ (0, 1)
                              ...
                              f (λx + (1 − λ)x0 ) ≥ λf (x) + (1 − λ)f (x0 ).        (3.2.2)
...
(equivalently)
                              ∀x, x0 ∈ X, λ ∈ [0, 1]
since they are satisfied as equalities ∀f when x = x0 , when λ = 0 and when
λ = 1.
...
Note that there is no longer any flexibility as regards allowing x = x0 or λ = 0 or
...
Definition 3.2.4 Consider the real-valued function f : X → Y where Y ⊆ <.
   ...
   1. The upper contour sets of f are the sets {x ∈ X : f (x) ≥ α} (α ∈ <).

   2. The level sets or indifference curves of f are the sets {x ∈ X : f (x) = α}
      (α ∈ <).

   3. The lower contour sets of f are the sets {x ∈ X : f (x) ≤ α} (α ∈ <).
...
be differentiable, X ⊆ <n an open, convex set. Then:
f is (strictly) concave
 ⇐⇒
∀x 6= x0 ∈ X,
                        f (x) ≤ (<)f (x0 ) + f 0 (x0 )(x − x0 ).           (3.2.3)
...
Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                               31


     (a) Suppose that f is concave.
         Then, for λ ∈ (0, 1),

                 f (x0 + λ(x − x0 )) ≥ f (x0 ) + λ (f (x) − f (x0 )) .          (3.2.4)

          Subtract f (x0 ) from both sides and divide by λ:
                   f (x0 + λ(x − x0 )) − f (x0 )
                                                 ≥ f (x) − f (x0 ).             (3.2.5)
                               λ
          Now consider the limits of both sides of this inequality as λ → 0.
          The LHS tends to f 0 (x0 ) (x − x0 ) by definition of a directional deriva-
          ...
          cave function.
     (b) Now suppose that f is strictly concave and x 6= x0 .
         Since f is also concave, we can apply the result that we have just
         proved to x0 and x00 ≡ 12 (x + x0 ) to show that

                             f 0 (x0 )(x00 − x0 ) ≥ f (x00 ) − f (x0 ).         (3.2.6)

          Using the definition of strict concavity (or the strict version of inequal-
          ity (3.2.4)) gives:
                                                   1
                            f (x00 ) − f (x0 ) >     (f (x) − f (x0 )) .        (3.2.7)
                                                   2
 ...
 2. Conversely, suppose that the derivative satisfies inequality (3.2.3). We will
    ...
    Set x0 = λx + (1 − λ)x00 . Then, applying the hypothesis of the proof in turn
    to x and x0 and to x00 and x0 yields:

                          f (x) ≤ f (x0 ) + f 0 (x0 )(x − x0 )                  (3.2.8)
    and
                         f (x00 ) ≤ f (x0 ) + f 0 (x0 )(x00 − x0 )              (3.2.9)
    ...
    A convex combination of (3.2.8) and (3.2.9) gives:
             ...
             λf (x) + (1 − λ)f (x00 )
                                                              ...
                                                              Revised: December 2, 1998
32                                          3.2. CONVEXITY AND CONCAVITY


                   ≤ f (x0 ) + f 0 (x0 ) (λ ((x − x0 )) + (1 − λ) ((x00 − x0 )))
                   = f (x0 ),                                                    (3.2.10)
        since
         λ ((x − x0 )) + (1 − λ) ((x00 − x0 )) = λx + (1 − λ)x00 − x0 = 0n . (3.2.11)
...
X → < be twice continuously differentiable (C 2 ), X ⊆ <n open and convex.
Then:

     1. f is concave
        ⇐⇒
        ∀x ∈ X, the Hessian matrix f 00 (x) is negative semidefinite.
     2. f 00 (x) negative definite ∀x ∈ X
        ⇒
        f is strictly concave.
...
variable.
     ...
     1. Suppose first that f 00 (x) is negative semi-definite ∀x ∈ X. Recall Taylor’s
        Theorem above (Theorem 2.8.1).
        It follows that f (x) ≤ f (x0 ) + f 0 (x0 )(x − x0 ). Theorem 3.2.3 shows that f
        is then concave. A similar proof will work for negative definite Hessian and
        strictly concave function.

Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                                 33
 ...
 2. To demonstrate necessity, we must consider separately first functions of a
    single variable and then functions of several variables.
     ...
     (a) First consider a function of a single variable. Instead of trying to show
         ...
         So suppose f 00 (x∗ ) > 0. Then, since f is twice continuously differ-
         entiable, f 00 (x) > 0 for all x in some neighbourhood of x∗ , say (a, b).
         Then f 0 is an increasing function on (a, b). Consider two points in
         (a, b), x < x00 and let x0 = λx + (1 − λ)x00 ∈ X, where λ ∈ (0, 1).
         Using the fundamental theorem of calculus,
                                          Z x0
                        0
                    f (x ) − f (x) =              f 0 (t)dt < f 0 (x0 )(x0 − x)
                                            x
        and                               Z x00
                   f (x00 ) − f (x0 ) =           f 0 (t)dt < f 0 (x0 )(x00 − x0 ).
                                            x0
     ...
     (b) Now consider a function of several variables. Suppose that f is con-
         cave and fix x ∈ X and h ∈ <n . (We use an x, x+h argument instead
         of an x, x0 argument to tie in with the definition of a negative definite
         ...
         that g 00 (0) = h> f 00 (x)h, so f 00 (x) is negative semi-definite.

                                                              Revised: December 2, 1998
34                                                 3.2. CONVEXITY AND CONCAVITY
     ...
     3. For functions of one variable, the above arguments can give an alternative
        ...
        have something like the following:
                   ...
                   f 00 (x) < 0 on (a, b)     ⇒     f locally strictly concave on (a, b)
                   f 00 (x) ≤ 0 on (a, b)     ⇒     f locally concave on (a, b)
                   f 00 (x) > 0 on (a, b)     ⇒     f locally strictly convex on (a, b)
                   f 00 (x) ≥ 0 on (a, b)     ⇒     f locally convex on (a, b)
...
Let X ⊆ <n be a convex set and f : X → < a real-valued function defined on X.
...
Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                                           35
...
siconcavity:
   ...
   1. ∀x 6= x0 ∈ X such that f (x0 ) ≤ f (x) and ∀λ ∈ (0, 1),

                                    f (λx + (1 − λ)x0 ) ≥ f (x0 ).

   2. ∀x, x0 ∈ X, λ ∈ [0, 1], f (λx + (1 − λ)x0 ) ≥ min{f (x), f (x0 )}.

   3. (If f is differentiable,) ∀x, x0 ∈ X such that f (x) − f (x0 ) ≥ 0, f 0 (x0 )(x −
      x0 ) ≥ 0.

Proof      1. We begin by showing the equivalence between the definition and
        condition 1.3

         (a) First suppose that the upper contour sets are convex. Let x and x0 ∈ X
             and let α = min{f (x), f (x0 )}. Then x and x0 are in C(α). By the
             hypothesis of convexity, for any λ ∈ (0, 1), λx + (1 − λ)x0 ∈ C(α).
             The desired result now follows.
         (b) Now suppose that condition 1 holds. To show that C(α) is a convex
             set, we just take x and x0 ∈ C(α) and investigate whether λx + (1 −
             λ)x0 ∈ C(α). But, by our previous result,

                             f (λx + (1 − λ)x0 ) ≥ min{f (x), f (x0 )} ≥ α

              where the final inequality holds because x and x0 are in C(α).

   2. It is almost trivial to show the equivalence of conditions 1 and 2.

         (a) In the case where f (x) ≥ f (x0 ) or f (x0 ) = min{f (x), f (x0 )}, there
             is nothing to prove. Otherwise, we can just reverse the labels x and x0 .
             The statement is true for x = x0 or λ = 0 or λ = 1 even if f is not
             quasiconcave.
   3
...
isation to act as definition.
                                                                    ...
                                                                    Revised: December 2, 1998
36                                          3.2. CONVEXITY AND CONCAVITY
     ...
     3. Proving that condition 3 is equivalent to quasiconcavity for a differentiable
        ...
        trickiest part of the proof.
         ...
         (a) Begin by supposing that f satisfies conditions 1 and 2. Proving that
             ...
             any λ ∈ (0, 1) and, without loss of generality, x and x0 such that
             f (x0 ) ≤ f (x). By quasiconcavity,

                                    f (λx + (1 − λ)x0 ) ≥ f (x0 ).            (3.2.12)

             Consider

                   f |L (λ) = f (λx + (1 − λ)x0 ) = f (x0 + λ(x − x0 )) .     (3.2.13)

             We want to show that the directional derivative

                                   f |0L (0) = f 0 (x0 )(x − x0 ) ≥ 0.        (3.2.14)
         ...
         (b) Now the difficult part — to prove that condition 3 is a sufficient con-
             ...
             not quasiconcave. In other words, ∃x, x0 , λ∗ such that

                        f (λ∗ x + (1 − λ∗ )x0 ) < min{f (x), f (x0 )},        (3.2.16)

             where without loss of generality f (x0 ) ≤ f (x). The hypothesis of the
             theorem applied first to x and λ∗ x + (1 − λ∗ )x0 and then to x0 and
             λ∗ x + (1 − λ∗ )x0 tells us that:

                     f 0 (λ∗ x + (1 − λ∗ )x0 ) (x − (λ∗ x + (1 − λ∗ )x0 )) ≥ 0 (3.2.17)
                    f 0 (λ∗ x + (1 − λ∗ )x0 ) (x0 − (λ∗ x0 + (1 − λ∗ )x)) ≥ 0. (3.2.18)

Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                                   37
               ...
               a pair of inequalities which can only be satisfied simultaneously if:
                                  ...
                                  f 0 (λ∗ x + (1 − λ∗ )x0 ) (x0 − x) = 0.         (3.2.19)

               In other words, f |0L (λ∗ ) = 0; we already know that f |L (λ∗ ) < f |L (0) ≤
...
cave function has a higher value at x than at x0 , or the same value at both points,
then the directional derivative of f at x0 in the direction of x is non-negative. It
might help to think about this by considering n = 1 and separating out the cases
x > x0 and x < x0 .

Theorem 3.2.7 Let f : X → < be quasiconcave and g : < → < be increasing.
...
Definition 3.2.6 f : X → < is strictly quasiconcave
 ⇐⇒
∀x 6= x0 ∈ X such that f (x) ≥ f (x0 ) and ∀λ ∈ (0, 1), f (λx + (1 − λ)x0 ) >
f (x0 ).
...
−f is (strictly) quasiconcave4
   4
       EC3080 ended here for Hilary Term 1998.
                                                               ...
                                                               Revised: December 2, 1998
38                                            3.2. CONVEXITY AND CONCAVITY
...
where M ∈ < and p ∈ <n . This function is both concave and convex, but neither
strictly concave nor strictly convex. Furthermore,

                   f (λx + (1 − λ) x0 ) = λf (x) + (1 − λ)f (x0 )                    (3.2.20)
                                        ≥ min{f (x), f (x0 )}                        (3.2.21)

and

            (−f ) (λx + (1 − λ) x0 ) = λ(−f )(x) + (1 − λ)(−f )(x0 )                 (3.2.22)
                                     ≥ min{(−f )(x), (−f )(x0 )},                    (3.2.23)
...
is, however, pseudoconcave (and pseudoconvex) since
                     ...
                     f (x) > f (x0 )    ⇐⇒       p> x < p> x0                        (3.2.24)
                                        ⇐⇒       p> (x − x0 ) < 0                    (3.2.25)
                                        ⇐⇒       −f 0 (x0 )(x − x0 ) < 0             (3.2.26)
                                        ⇐⇒       f 0 (x0 )(x − x0 ) > 0.             (3.2.27)

Finally, here are two graphs of Cobb-Douglas functions:5
     5
    To see them you will have to have copied two .WMF files retaining their uppercase filenames
and put them in the appropriate directory,
C:/TCD/teaching/WWW/MA381/NOTES/!


Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                            39




                               Graph of z = x0.5 y 0.5




                              Graph of z = x−0.5 y 1.5

3.3     Unconstrained Optimisation
Definition 3.3.1 Let X ⊆ <n , f : X → <.
Then f has a (strict) global maximum at x∗ ⇐⇒ ∀x ∈ X, x 6= x∗ , f (x) ≤ (<
)f (x∗ ).
Also f has a (strict) local maximum at x∗ ⇐⇒ ∃ > 0 such that ∀x ∈
B(x∗ ), x 6= x∗ , f (x) ≤ (<)f (x∗ ).

Similarly for minima.

Theorem 3.3.1 A continuous real-valued function on a compact subset of <n at-
...
are generally for functions on open domains, since the limit of the first difference
                                                         ...
                                                         Revised: December 2, 1998
40                                     3.3. UNCONSTRAINED OPTIMISATION
...
lem
                                   max f (x)                              (3.3.1)
                                      x∈X
                 n
where X ⊆ < and f : X → < is a real-valued function of several variables,
...
Then f 0 (x∗ ) = 0, or f has a stationary point at x∗ .

Proof Without loss of generality, assume that the function has a local maximum
at x∗ . Then ∃ > 0 such that, whenever khk < ,
...
It follows that, for 0 < h < ,
                              f (x∗ + hei ) − f (x∗ )
                                       ...
                                       h
Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                              41
...
minima.
Let X ⊆ <n be open and let f : X → < be a twice continuously differentiable
function with f 0 (x∗ ) = 0 and f 00 (x∗ ) negative definite.
...
of Theorem 3.2.4: for any x ∈ X, ∃s ∈ (0, 1) such that
                                        ...
                                        1
  f (x) = f (x∗ ) + f 0 (x∗ )(x − x∗ ) + (x − x∗ )> f 00 (x∗ + s(x − x∗ ))(x − x∗ ).
                                        2
or, since the first derivative vanishes at x∗ ,
                             1
            f (x) = f (x∗ ) + (x − x∗ )> f 00 (x∗ + s(x − x∗ ))(x − x∗ ).
                             2
Since f 00 is continuous, f 00 (x∗ + s(x − x∗ )) will also be negative definite for x
in some open neighbourhood of x∗ . Hence, for x in this neighbourhood, f (x) <
f (x∗ ) and f has a strict local maximum at x∗ .
                                                             Revised: December 2, 1998
42                                    3.3. UNCONSTRAINED OPTIMISATION
...
Proof Suppose not, in other words that ∃x 6= x∗ such that f (x) = f (x∗ ).
Then, for any α ∈ (0, 1),
...
x ∈ B (x∗ ). If there are points at which the Hessian is merely semi-definite, then
...
Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                                         43
...
lem
                                         maxx∈X f (x)
                                       s.t. g (x) = 0m                                   (3.4.1)
where X ⊆ <n , f : X → < is a real-valued function of several variables, called
the objective function of Problem (3.4.1) and g : X → <m is a vector-valued
function of several variables, called the constraint function of Problem (3.4.1); or,
equivalently, g j : X → < are real-valued functions for j = 1, . . . , m. In other
...
g 1 , . . . , g m are all once or twice continuously differentiable.
...
Note that
   6
       As usual, a row of numbers separated by commas is used as shorthand for a column vector.
                                                                    ...
                                                                    Revised: December 2, 1998
                             3.4. EQUALITY CONSTRAINED OPTIMISATION:
44                                 THE LAGRANGE MULTIPLIER THEOREMS
...
The Lagrange multiplier method involves the following four steps:
     ...
     2. Define the Lagrangean L: X × <m → <, where
     ...
     3. Find the stationary points of the Lagrangean, i.e. set L0 (x, λ) = 0. Since
...
Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                                45


along the tangent) or f 0 (x) (x0 − x) = 0, the gradient vector, f 0 (x)> , must be
perpendicular to the direction of the tangent, x0 − x.
At the optimum, the level sets of f and g have a common tangent, so f 0 (x) and
g 0 (x) are collinear, or f 0 (x) = −λg 0 (x). It can also be seen with a little thought
...
then ∃λ∗ ∈ <m such that f 0 (x∗ ) + λ∗> g 0 (x∗ ) = 0 (i.e. in <n , f 0 (x∗ ) is in the
                                                          0                0
m−dimensional subspace generated by the m vectors g 1 (x∗ ) , . . . , g m (x∗ )).
...
Theorem. Using this theorem, we must find the m weights λ1 , . . . , λm to prove
                                             0                   0
that f 0 (x∗ ) is a linear combination of g 1 (x∗ ) , . . . , g m (x∗ ).
Without loss of generality, we assume that the first m columns of g 0 (x∗ ) are lin-
...
on Z such that
                               g (h (z) , z) = 0 ∀z ∈ Z
                                                             ...
                                                             Revised: December 2, 1998
                              3.4. EQUALITY CONSTRAINED OPTIMISATION:
46                                  THE LAGRANGE MULTIPLIER THEOREMS


and also
                                  h0 (z∗ ) = − (Dy g)−1 Dz g.
Now define a new objective function F : Z → < by
...
Substituting for h0 (z) gives:

                                  Dy f (Dy g)−1 Dz g = Dz f.

We can also partition f 0 (x) as
                                                                 
                                  Dy f (Dy g)−1 Dy g Dz f

Substituting for the second sub-matrix yields:
                                                                     
              f 0 (x) =       Dy f (Dy g)−1 Dy g Dy f (Dy g)−1 Dz g
                                                             
                      = Dy f (Dy g)−1            Dy g Dz g
                      = −λ > g 0 (x)
...
Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                                   47


   2. f 0 (x∗ ) + λ∗> g 0 (x∗ ) = 0 (i.e. the first order conditions are satisfied at x∗ ),

   3. λ∗j ≥ 0 for j = 1, . . . , m,

   4. f is pseudoconcave, and

   5. g j is quasiconcave for j = 1, . . . , m,
...
Since x∗ is not a maximum, ∃x 6= x∗ such that g (x) = 0 but f (x) > f (x∗ ).
By pseudoconcavity, f (x) − f (x∗ ) > 0 implies that f 0 (x∗ ) (x − x∗ ) > 0.
Since the constraints are satisfied at both x and x∗ , we have g (x∗ ) = g (x) = 0.
By quasiconcavity of the constraint functions (see Theorem 3.2.6), g j (x)−g j (x∗ ) =
                  0
0 implies that g j (x∗ ) (x − x∗ ) ≥ 0.
By assumption, all the Lagrange multipliers are non-negative, so

                    f 0 (x∗ ) (x − x∗ ) + λ∗> g 0 (x∗ ) (x − x∗ ) > 0.
...
If
   ...
   3. g j is an affine function (i.e. both convex and concave) for j = 1, . . . , m,

then x∗ is the unique (global) maximum.

                                                                  Revised: December 2, 1998
                            3.4. EQUALITY CONSTRAINED OPTIMISATION:
48                                THE LAGRANGE MULTIPLIER THEOREMS
...
require any differentiability assumption.
     • We first show that the feasible set is convex.
       Suppose x 6= x∗ are two distinct solutions.
       Consider the convex combination of these two solutions xα ≡ αx+(1 − α) x∗ .
       Since each g j is affine and g j (x∗ ) = g j (x) = 0, we have

                        g j (xα) = αg j (x) + (1 − α) g j (x∗ ) = 0.
...
sation problem:
                     ...
                     max f (x, α)     subject to g (x, α) = 0,                (3.4.2)
                      x

where x ∈ <n , α∈ <q , f : <n+q → < and g: <n+q → <m (i.e. as usual f is
...
Let x∗ (α) denote the optimal choice of x for given α (x∗ : <q → <n is called the
optimal response function) and let M (α) denote the maximum value attainable
by f for given α (M : <q → < is called the envelope function).
...
Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                            49
...
make sufficient assumptions to guarantee that
   ...
   2. the Hessian f 00 (x∗ ) is a negative definite (n × n) matrix; and
...
problem
                                   ...
                                   maxx∈X f (x)
                        s.t. g i (x) ≥ 0, i = 1, 2, . . . , m               (3.5.1)

where once again X ⊆ <n , f : X → < is a real-valued function of several
variables, called the objective function of Problem (3.5.1) and g : X → <m
...
(as illustrated, assuming f quasiconcave as usual and hi quasiconvex or g i quasi-
concave, so that the relationship between αi and λi is negative)

                                    hi (x) ≤ αi                             (3.5.5)

Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                                 51

                          Derivative of Lagrange Constraint
  Type of constraint      objective fn. Multiplier  fn.
                              0
       Binding/active        f ≤0        λ≥0       g=0
                              0
 Non-binding/inactive        f =0        λ=0       g>0
        Table 3.1: Sign conditions for inequality constrained optimisation

(or g i (x, α) ≥ 0). For values of αi such that λi > 0, this constraint is strictly
...
If
   1. x∗ solves Problem (3.5.1), with
      ...
      necessary to achieve this),
                                                                ...
                                                                Revised: December 2, 1998
                          3.5. INEQUALITY CONSTRAINED OPTIMISATION:
52                                        THE KUHN-TUCKER THEOREMS


     2. f and g are continuously differentiable, and

     3. the b × n submatrix of g 0 (x∗ ),
                                     ∂g 1                ∂g 1
                                                                  
                                     ∂x1
                                          (x∗ )   ...    ∂xn
...
then ∃λ ∈ <m such that f 0 (x∗ ) + λ> g 0 (x∗ ) = 0, with λi ≥ 0 for i = 1, 2, . . . , m
and g i (x∗ ) = 0 if λi > 0.
...
case. It can be broken into seven steps.
     1. Suppose x∗ solves Problem (3.5.1).
        ...
        problem:
                                         ...
                                         maxx∈B (x∗ ) f (x)
                              s.t.   g i (x) ≥ 0, i = 1, 2, . . . , b.         (3.5.10)

        In other words, since the non-binding constraints are non-binding ∀x ∈
        B (x∗ ) by construction, we can ignore them if we confine our search for
...
Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                               53


                                    s.t. G (x, s) = 0b                         (3.5.11)

    where Gi (x, s) ≡ g i (x) − si , i = 1, 2, . . . , b.
...
54                                        THE KUHN-TUCKER THEOREMS
     ...
     5. The Kuhn-Tucker multipliers can now be found exactly as in the Lagrange
        case. We know that
                          Dz F = Dy f Dz h + Dz f In−b = 0n−b .
        Substituting for Dz h from (3.5.19) gives:
                                      Dy f (Dy g)−1 Dz g = Dz f.
     ...
     6. Next, we calculate the partial derivatives of F with respect to the slack
        ...
        of (3.5.21) with respect to s to obtain:
                             Ds F = Dy f Ds h + Dz f 0(n−b)×b                 (3.5.24)
                                                         −1
                                        = Dy f (Dy g)                         (3.5.25)
                                        = −λ ,                                (3.5.26)
...
Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                                       55
...
If
   ...
   2. ∃λ ∈ <m such that f 0 (x∗ )+λ> g 0 (x∗ ) = 0, with λi ≥ 0 for i = 1, 2, . . . , m
      and g i (x∗ ) = 0 if λi > 0 (i.e. the first order conditions are satisfied at x∗ ),

   3. f is pseudoconcave, and

   4. g i (x∗ ) = 0, i = 1, 2, . . . , b; g i (x∗ ) > 0, i = b + 1, . . . , m and g j is qua-
...
If
   ...
   3. g j is a quasiconcave function for j = 1, . . . , m,

then x∗ is the unique (global) optimal solution.

                                                                      Revised: December 2, 1998
                                 3.5. INEQUALITY CONSTRAINED OPTIMISATION:
56                                               THE KUHN-TUCKER THEOREMS
...
strained optimisation problem:
               max
                x
                   f (x, α)        subject to g i (x, α) ≥ 0, i = 1, . . . , m   (3.5.28)

where x ∈ <n , α∈ <q , f : <n+q → < and g: <n+q → <m .
Let x∗ (α) denote the optimal choice of x for given α (x∗ : <q → <n ) and let
M (α) denote the maximum value attainable by f for given α (M : <q → <).
If
     1. f is continuous;
...
Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                              57


 1. The canonical quadratic programming problem.
   Find the vector x ∈ <n which maximises the value of the quadratic form
   x> Ax subject to the m linear inequality constraints gi> x ≥ αi , where
   A ∈ <n×n is negative definite and gi ∈ <n for i = 1, . . . , m.
   ...
   symmetric (negative definite) matrixl, since as x> Ax is a scalar,
                            ...
                            x> Ax =       x> Ax                               (3.5.29)
                                 ...
                                 = x       A+A      x                         (3.5.32)
                                       2
                  
   and 21 A + A> is always symmetric.
   ...
   The first order conditions are:
                                2x> A + λ> G = 0n                             (3.5.34)
   or, transposing and multiplying across by 12 A−1 :
                                      1
                                 x = − A−1 G> λ.                              (3.5.35)
                                      2
   If the constraints are binding, then we will have:
                                ...
                                α = − GA−1 G> λ.                              (3.5.36)
                                     2
   Now we need the fact that G (and hence GA−1 G> ) has full rank to solve
...
58                                                                     3.6. DUALITY
        ...
        In the case in which all the constraints are binding, the solution is:
                                                        −1
                                 x = A−1 G> GA−1 G>            α                 (3.5.39)

        and the envelope function is given by:
                                        −1                               −1
            x> Ax = α> GA−1 G>                 GA−1 AA−1 G> GA−1 G>                α
                                        −1
                     = α> GA−1 G>              α
                        1
                     = − α> λ.                                                   (3.5.40)
...
3.6 Duality
Let X ⊆ <n and let f, g : X 7→ < be, respectively, pseudoconcave and pseudo-
...
Revised: December 2, 1998
CHAPTER 3. CONVEXITY AND OPTIMISATION                                          59
...
The first order conditions for the two problems are respectively:
                              ...
                              f 0 (x) − λg 0 (x) = 0                       (3.6.3)

and
                              g 0 (x) + µf 0 (x) = 0,                      (3.6.4)
where λ and µ are the relevant Lagrange multipliers.
Thus if x and λ∗ 6= 0 solve (3.6.3), then x and µ∗ ≡ −1/λ solve (3.6.4). However,
...
APPLICATIONS
             ...
             Revised: December 2, 1998
CHAPTER 4. CHOICE UNDER CERTAINTY                                                            63
...
a consumption vector or consumption plan or consumption bundle, xh = x1h , . . . , xNh ,
                                                                    N
from a (closed, convex) consumption set, Xh . Typically, Xh = <+ . More gener-
ally, consumer h’s consumption set might require a certain subsistence consump-
tion of some commodities, such as water, and rule out points of <N   + not meeting
this requirement. The household’s endowments are denoted eh ∈ <N      + and can be
traded.
In a production economy, the shareholdings of households in firms are denoted
ch ∈ <F ,
A consumer’s net demand is denoted zh ≡ xh − eh ∈ <N .
...
Revised: December 2, 1998
CHAPTER 4. CHOICE UNDER CERTAINTY                                                       67
...
Proof For the proof of this theorem, see ?.                                       Q.E.D.
Axiom 5 (Greed) Greed is incorporated into consumer behaviour by assuming
either
   1. Strong monotonicity:
      If X = <N+ , then  is said to be strongly monotonic iff whenever xn ≥ yn ∀n
      but x 6= y, x  y. [x = (x1 , . . . , xN ) &c.]; or
   2. Local non-satiation:
      ∀x ∈ X ,  > 0, ∃x0 ∈ B (x) s.t. x0  x
...
theorem.
Theorem 4.3.2 (Varian) If X = <N    + and  is a complete, reflexive, transitive,
...
By strong monotonicity, the sets {t ∈ < : t1  x} and {t ∈ < : x  t1} are both
non-empty.
...
By connectedness of <, they intersect in at least one point, u(x) say, and x ∼
u(x)1.
Now
                           xy       ⇐⇒      u(x)1  u(y)1
                                     ⇐⇒      u(x) ≥ u(y)
...
Proof In either case, both statements are equivalent to saying that
                 ...
                 u(x) ≥ u(y) =⇒ u (λx + (1 − λ) y) ≥ (>)u(y).                 (4.3.1)

Revised: December 2, 1998
CHAPTER 4. CHOICE UNDER CERTAINTY                                                    69
...
ings ch ∈ <F and preference ordering h represented by utility function uh who
desires to trade his endowment at prices p ∈ <N
                                              + faces an inequality constrained
optimisation problem:

                  max uh (x) s.t. p> x ≤ p> eh + c>
                                                  h Π (p) ≡ Mh                  (4.4.1)
                 x∈Xh
...
by M in either case.
                                                            ...
                                                            Revised: December 2, 1998
                                       4.4. OPTIMAL RESPONSE FUNCTIONS:
70                                   MARSHALLIAN AND HICKSIAN DEMAND
...
identify a maximum.
The Lagrangian, using multipliers λ for the budget constraint and µ ∈ <N for the
non-negativity constraints, is
                                                     
                            uh (x) + λ M − p> x + µ> x.                               (4.4.2)

The first order conditions are given by the (N -dimensional) vector equation:

                              u0h (x) + λ (−p) + µ = 0N                               (4.4.3)
...
Now for each p ∈ <N    ++ (ruling out bads, or goods with negative prices, and even
(see below) free goods), eh ∈ Xh , and ch ∈ <F , or for each p, Mh combination,
...
Revised: December 2, 1998
CHAPTER 4. CHOICE UNDER CERTAINTY                                                 71
...
Other noteworthy properties of Marshallian demand include the following:
   ...
   2. If p includes a zero price (pn = 0 for some n), then xh (p, Mh ) may not be
      well defined.
                                                          ...
                                                          Revised: December 2, 1998
                                    4.4. OPTIMAL RESPONSE FUNCTIONS:
72                                MARSHALLIAN AND HICKSIAN DEMAND
        ...
        to define Marshallian demand only on the open non-negative orthant in <N ,
        namely <N ++ .
...
dence between income M and utility ū for a given price vector p. The expenditure
...
Revised: December 2, 1998
CHAPTER 4. CHOICE UNDER CERTAINTY                                                 73
...
prove extremely useful later on:
                       e (p, v (p, M ))    =   M                              (4.4.6)
                        v (p, e (p, ū))   =   ū                             (4.4.7)
                              x (p, M )    =   h (p, v (p, M ))               (4.4.8)
                               h (p, ū)   =   x (p, e (p, ū))               (4.4.9)
...
must, by continuity, be a cheaper consumption vector still yielding utility ū.
...
Revised: December 2, 1998
CHAPTER 4. CHOICE UNDER CERTAINTY                                                75
...
The following are interesting properties of the expenditure function:
   ...
   3. The expenditure function is concave in prices.
      To see this, we just fix two price vectors p and p0 and consider the value of
      the expenditure function at the convex combination pλ ≡ λp + (1 − λ) p0 .


                e (pλ, ū) = (pλ)> h (pλ, ū)                                (4.5.5)
                                  >                            0 >
                           = λp h (pλ, ū) + (1 − λ) (p ) h (pλ, ū)         (4.5.6)
                                                              0 >
                           ≥ λp> h (p, ū) + (1 − λ) (p ) h (p0 , ū)        (4.5.7)
                           = λe (p, ū) + (1 − λ) e (p0 , ū) ,              (4.5.8)
...
Theorem 4.6.1 (Shephard’s Lemma.)
                  ∂eh            ∂  >                    
                      (p, ū) =     p  x + λ (uh (x) − ū)                   (4.6.1)
                  ∂pn           ∂pn
                              = xn                                           (4.6.2)
which, when evaluated at the optimum, is just hnh (p, ū).
...
prices are the corresponding Hicksian demand functions.
                                                          ...
                                                          Revised: December 2, 1998
76                            4.6. FURTHER RESULTS IN DEMAND THEORY
...
implies that
               ...
               ∂v                   ∂v               ∂e
                 n
                   (p, e (p, ū)) +    (p, e (p, ū)) n (p, ū) = 0            (4.6.4)
               ∂p                   ∂M               ∂p
and using Shephard’s Lemma gives:

                ∂v                   ∂v
                  n
                    (p, e (p, ū)) +    (p, e (p, ū)) hn (p, ū) = 0          (4.6.5)
                ∂p                   ∂M
Hence
                                            ∂v
                            n              ∂pn
...
than the corresponding value of utility ū:
                                                ∂v
                                n              ∂pn
                                                   (p, M )
                            x       (p, M ) = − ∂v         .                   (4.6.7)
...
Revised: December 2, 1998
CHAPTER 4. CHOICE UNDER CERTAINTY                                                    77
...
and hence that
           ...
           ∂eh          ∂eh
Since hm            n
       h = ∂pm and hh = ∂pn , and the result follows.
                                                                                Q.E.D.
...
[This is still on a handwritten sheet.]
                                                               ...
                                                               Revised: December 2, 1998
78                                        4.7. GENERAL EQUILIBRIUM THEORY
...
Differentiating the RHS of (4.4.9) with respect to pn yields:
                 ∂xm                   ∂xm               ∂e
                    n
                      (p, e (p, ū)) +     (p, e (p, ū)) n (p, ū) .                 (4.6.13)
                 ∂p                    ∂M                ∂p
To complete the proof:
                              m
     1. set this equal to ∂h
                          ∂pn
                              (p, ū)
     2. substitute from Shephard’s Lemma
     3. define M ≡ e (p, ū) (which implies that ū ≡ V (p, M ))
                                                                                      Q.E.D.
...
other worse off.
                                                                                 PH
Definition 4.8.2 X is Pareto dominated by X0 = (x01 , . . . , x0H ) if             h=1 xh   =
PH     0   0                0
 h=1 xh , xh h xh ∀h and xh h xh for at least one h.
     3
...
Revised: December 2, 1998
CHAPTER 4. CHOICE UNDER CERTAINTY                                                             79
...
which is Pareto dominated by a feasible allocation X0 .
If individual h is strictly better off under X0 or x0h h xh , then it follows that
individual h cannot afford x0h at the equilibrium prices p or
                              p> x0h > p> xh = p> eh .                                    (4.8.1)
The latter equality is just the budget constraint, which is binding since we have
assumed local non-satiation.
Similarly, if individual h is indifferent between X and X0 or x0h ∼h xh , then it
follows that
                              p> x0h ≥ p> xh = p> eh ,                     (4.8.2)
since if x0h cost strictly less than xh , then by local non-satiation some consumption
vector near enough to x0h to also cost less than xh would be strictly preferred to
...
But since X0 is feasible we must have for each good n
                                    H              H
                                          x0n            enh
                                    X              X
                                           h ≤
...
cation X0h can exist.
                                                                           Q.E.D.
Before proceeding to the second welfare theorem, we need to say a little bit about
separating hyperplanes.

                                                                        Revised: December 2, 1998
80                                                4.8. THE WELFARE THEOREMS


4.8.4 The Separating Hyperplane Theorem
                            n                         o
Definition 4.8.3 The set z ∈ <N : p> z = p> z∗ is the hyperplane through z∗
with normal p.

Note that any hyperplane divides <N into two closed half-spaces,
                                n                           o
                                    z ∈ <N : p> z ≤ p> z∗
...
<N and z∗ ∈ Z, z∗ 6∈ int Z, then ∃p∗ 6= 0 in <N such that p∗> z∗ ≤ p∗> z ∀z ∈ Z,
...
(x∗g
  h > 0 ∀g = 1, . . . , N ; h = 1, . . . , H), then a reallocation of the initial ag-
gregate endowment can yield an equilibrium where the allocation is X∗ .

Revised: December 2, 1998
CHAPTER 4. CHOICE UNDER CERTAINTY                                                                       81
...
Proof There are four main steps in the proof.
   ...
   1. First we construct a set of utility-enhancing endowment perturbations, and
      ...
      dowment perturbation is affordable.
       ...
       is also a convex set.
                                  endowment x∗ = H        ∗
                                                         P
       Given an aggregate initial
                              PH                     h=1 xh , we interpret any vec-
                                          ∗
       tor of the form z = h=1 xh − x as an endowment perturbation. Now
   ...
   2. Next, we need to show that the zero vector is in the set Z, but not in the
      interior of Z.
                                                                                    PH
       To show that 0 ∈ Z, we just set xh = x∗h and observe that 0 =                       ∗
                                                                                      h=1 xh −
       x∗ .4
...
Revised: December 2, 1998
CHAPTER 4. CHOICE UNDER CERTAINTY                                                               83
...
not be used with merely quasiconcave utility functions.
                                                                         ...
                                                                         Revised: December 2, 1998
84                             4.9. MULTI-PERIOD GENERAL EQUILIBRIUM
...
Revised: December 2, 1998
CHAPTER 5. CHOICE UNDER UNCERTAINTY                                              85
...
function if
   ...
   1.    (a) Ω ∈ A
         (b) A ∈ A ⇒ Ω − A ∈ A
                                             S∞
         (c) Ai ∈ A for i = 1, . . . , ∞ ⇒   i=1 Ai ∈ A

        (i.e. A is a sigma-algebra of events)

                                                          Revised: December 2, 1998
86                                           5.2. REVIEW OF BASIC PROBABILITY
     ...
     2.    (a) P (Ω) = 1
           ...
           (c) P ( ∞
                  S               P∞
                    i=1 Ai ) =      i=1 P (Ai ) when A1 , A2 , . . . are pairwise disjoint
               events in A.
...
The function x̃ : Ω → < is a random variable (r.v.) if ∀x ∈ < {ω ∈ Ω : x̃ (ω) ≤
...
The function Fx̃ : < → [0, 1] : x 7→ Pr (x̃ ≤ x) ≡ P ({ω ∈ Ω : x̃ (ω) ≤ x}) is
known as the cumulative distribution function (c.d.f.) of the random variable x̃.
...
by time, e.g. {x̃t : t ∈ T } or just {x̃t } if the time interval is clear from the context.
...
average) of a discrete r.v., x̃, with possible values x1 , x2 , x3 , . . . is given by
                                            ∞
                                            X
                                 E [x̃] ≡         xi P r (x̃ = xi ) .
                                            i=1
...
Revised: December 2, 1998
CHAPTER 5. CHOICE UNDER UNCERTAINTY                                                          87


The covariance of two random variables x̃ and ỹ is given by

                        Cov [x̃, ỹ] ≡ E [(x̃ − e [x̃]) (ỹ − E [ỹ])] .
...
variables.
Given any two random variables x̃ and ỹ, we can define a third random variable˜
by
                                ˜ ≡ ỹ − α − β x̃.                       (5.2.1)
...
implicitly by imposing (two) conditions on˜. We do the latter by insisting
   ...
   1. ˜ and x̃ are uncorrelated (this is not the same as assuming statistical inde-
...
But what about the conditional expectation, E [ỹ|x̃ = x]? This is not equal to
α + βx, as one might expect, unless E [˜|x̃ = x] = 0. This requires statistical
independence rather than the assumed lack of correlation. Again, a sufficient
condition is multivariate normality.
The notion of the β of ỹ with respect to x̃ as given in (5.2.2) will recur frequently.
...
This might be a good place to talk about the MVN distribution and Stein’s lemma.
                                                                  ...
                                                                  Revised: December 2, 1998
88                         5.3. TAYLOR’S THEOREM: STOCHASTIC VERSION
...
where
                                mn (x̃) ≡ E [(x̃ − E[x̃])n ] .                           (5.3.3)
In particular,
                                         h                i
                       m1 (x̃) = E (x̃ − E[x̃])1 ≡ 0                                     (5.3.4)
                                         h                i
                       m2 (x̃) = E (x̃ − E[x̃])2 ≡ Var [x̃]                              (5.3.5)
                                         h                i
                       m3 (x̃) = E (x̃ − E[x̃])3 ≡ Skew [x̃]                             (5.3.6)
and                                      h                i
                       m4 (x̃) = E (x̃ − E[x̃])4 ≡ Kurt [x̃] ,                           (5.3.7)

which allows us to start the summation in (5.3.2) at n = 2 rather than n = 1.
Indeed, we can rewrite (5.3.2) as
                                 1                       1
          E[f (x̃)] = f (E[x̃]) + f 00 (E[x̃])Var [x̃] + f 000 (E[x̃])Skew [x̃]
                                 2                 ∞
                                                         6
                         1 0000                         1 (n)
                                                         f (E[x̃])mn (x̃). (5.3.8)
...
Revised: December 2, 1998
CHAPTER 5. CHOICE UNDER UNCERTAINTY                                                       89
...
sented by a column vector, yj ∈ <M , where yij is the payoff in state i of security
j.
...
complex security in each of the M states of nature, i.e.
   ...
   3
       Or maybe I mean its transpose.
                                                                    ...
                                                                    Revised: December 2, 1998
90                               5.4. PRICING STATE-CONTINGENT CLAIMS
...
5.4.2 Restrictions on security values implied by allocational ef-
      ...
      Cω = aggregate consumption in state ω
      
Let
      Ωk = {ω ∈ Ω : Cω = k}
...
where aggregate consumption equals k.
                                                                      ...
                                                                      Revised: December 2, 1998
92                                 5.4. PRICING STATE-CONTINGENT CLAIMS
...
Therefore, an arbitrary security x has value:
                          ...
                          Sx =             φωxω                             (5.4.10)
                               ...
                               =          φ(k)             xω               (5.4.13)
                                     k           ω∈Ωk π(k)
                                     X
                               =          φ(k)E[x̃|C̃ = k]                  (5.4.14)
                                     k


5.4.3 Completing markets with options on aggregate consump-
      tion
...
Revised: December 2, 1998
CHAPTER 5. CHOICE UNDER UNCERTAINTY                                             93
...
vector, x ∈ <k , for each time and state of the world.
...
values of the lotteries in L.
                                                          ...
                                                          Revised: December 2, 1998
94                                 5.5. THE EXPECTED UTILITY PARADIGM
...
Axiom 8 (Substitution or Independence Axiom) If a ∈ (0, 1] and p̃  q̃, then

                     ap̃ ⊕ (1 − a) r̃  aq̃ ⊕ (1 − a) r̃ ∀r̃ ∈ L.

Axiom 9 (Archimedian Axiom) If p̃  q̃  r̃ then

             ∃a, b ∈ (0, 1) s.t. ap̃ ⊕ (1 − a) r̃  q̃  bp̃ ⊕ (1 − b) r̃.
...
Revised: December 2, 1998
CHAPTER 5. CHOICE UNDER UNCERTAINTY                                                    95


5.5.2 Existence of expected utility functions
A function u: <k×|T | → < can be thought of as a utility function on sure things.

Definition 5.5.1 Let V : L → < be a utility function representing the preference
relation .
...
From the Archimedean axiom, it can be deduced that for every other lottery, p̃,
there exists a unique V (p̃) such that
...
We leave it as an exercise to deduce from the axioms that if x̃ ∼ ỹ and z̃ ∼ t̃ then
                          πx̃ ⊕ (1 − π) z̃ ∼ π ỹ ⊕ (1 − π) t̃.
Define z̃ ≡ πx̃ ⊕ (1 − π) ỹ.
Then, using the definitions of V (x̃) and V (ỹ),
z̃ ∼ πx̃ ⊕ (1 − π) ỹ
                                                                              
     ∼ π V (x̃) p+ ⊕ (1 − V (x̃)) p− ⊕ (1 − π) V (ỹ) p+ ⊕ (1 − V (ỹ)) p−
     = (πV (x̃) + (1 − π) V (ỹ)) p+ ⊕ (π (1 − V (x̃)) + (1 − π) (1 − V (ỹ))) p−
It follows that
                   V (πx̃ ⊕ (1 − π) ỹ) = πV (x̃) + (1 − π) V (ỹ) .
...
theh expected
       i value   ofh theirandom variable.
E u W̃ ≤ u E W̃ when u is concave
...
general and rigorous proof. Without loss of generality, consider the concave case.
   ...
   1. One can reinterpret the defining inequality (3.2.1) in terms of a discrete
      random vector x̃ taking on the value x with probability π and x0 with prob-
      ability 1 − π:
      ∀x 6= x0 ∈ X, π ∈ (0, 1)

                     f (πx + (1 − π)x0 ) ≥ πf (x) + (1 − π)f (x0 ),            (5.6.1)
   ...
   2. Using a similar approach to that used with the Taylor series expansion in
      ...
      the mean E [x̃] and a generic value x̃:

                      f (x̃) ≤ f (E [x̃]) + f 0 (E [x̃]) (x̃ − E [x̃]) .       (5.6.3)
   ...
   3. One can also appeal to the second order condition for concavity, and the
      second order Taylor series expansion of f around E [x̃]:

                                              1
                       E[f (x̃)] = f (E[x̃]) + f 00 (x∗ )Var [x̃] ,            (5.6.5)
                                              2
                                                             Revised: December 2, 1998
98                 5.6. JENSEN’S INEQUALITY AND SIEGEL’S PARADOX


      for some x∗ in the support of x̃. However, this supposes that x∗ is fixed,
      whereas in fact it varies with the value taken on by x̃, and is itself a random
      variable, correlated with x̃.
...
This result is often useful with functions such as x 7→ ln x and x 7→ x1 .
To get a feel for the extent to which E[f (x̃)] differs from f (E[x̃]), we can again
use the following (wrong) second order Taylor approximation based on (5.3.8):

                                            1
                     E[f (x̃)] ≈ f (E[x̃]) + f 00 (E[x̃])Var [x̃] .           (5.6.7)
                                            2
This shows that the difference is larger the larger is the curvature of f (as measured
by the second derivative at the mean of x̃) and the larger is the variance of x̃.
...
Proof Let Ft be the current forward price and S̃t+1 the unknown future spot price.
If                                 h     i
                                Et S̃t+1 = Ft ,
...
except in the degenerate case where S̃t+1 is known with certainty at time t.
...
Revised: December 2, 1998
CHAPTER 5. CHOICE UNDER UNCERTAINTY                                              99
...
The original and most obvious application
                                       n o of Siegel’s
                                                  n oparadox is in the case of
currency exchange rates. In that case, F̃t and S̃t are stochastic processes
...
risk averse at w if u00 (w) < 0 and globally risk averse if u00 (w) < 0 ∀w.
...
u0 (w) alone is meaningless, as u and u0 can be multiplied by any positive constant
...
Revised: December 2, 1998
CHAPTER 5. CHOICE UNDER UNCERTAINTY                                                 101
...
Here are some examples of utility functions and their risk measures:
   ...
   • Narrow power utility (CRRA, DARA):
                                    ...
                                    B      1
                       u(w) =          w1− B , w > 0, B > 0, B 6= 1
                                   B−1

                                                            Revised: December 2, 1998
102                                        5.8. THE MEAN-VARIANCE PARADIGM
...
for analysis of the portfolio choice problem:
   1. Taylor-approximated utility functions (see Section 2.8):
                        u(W̃ ) = u(E[W̃ ]) + u0 (E[W̃ ])(W̃ − E[W̃ ])
                                 1 00
                               +   u (E[W̃ ])(W̃ − E[W̃ ])2 + R3                     (5.8.1)
                                 2
                                  ∞
                                      1 (n)
                                        u (E[W̃ ])(W̃ − E[W̃ ])n
                           ...
                           R3 =                                                      (5.8.2)
                                 n=3 n!
   4
       Add comments: u0 (w) > 0, u00 (w) < 0 and u0 (w) → 1/w as B → 1, so that u(w) → ln w.
   5
       Check ? for details of this one.


Revised: December 2, 1998
CHAPTER 5. CHOICE UNDER UNCERTAINTY                                               103


      which implies:
                                       1
                E[u(W̃ )] = u(E[W̃ ]) + u00 (E[W̃ ])σ 2 (W̃ ) + E[R3 ]         (5.8.3)
                                       2
                  where      ∞
                                1 (n)
                                   u (E[W̃ ])mn (W̃ )
   ...
   2. Quadratic utility:
                                         b
                    E[u(W̃ )] = E[W̃ ] − E[W̃ 2 ]                              (5.8.5)
                                         2
                                         b                      
                              = E[W̃ ] −    (E[W̃ ])2 + σ 2 (W̃ )              (5.8.6)
                                         2
                                            b
                              = u(E[W̃ ]) − Var[W̃ ]                           (5.8.7)
                                            2
...
Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                      105
...
106                                        6.2. NOTATION AND PRELIMINARIES
        ...
        Weekly            £100 → £100 × 1 + .10
                                             52
                                                 ...
                                                             365
                                               .10
        Daily             £100 → £100 × 1 + 365      = £110.515. . .
                                          0.10
        Continuously      £100 → £100 × e      = £110.517. . .
...
Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                        107
 ...
 3. Aggregating and averaging returns across portfolios (r̃w = w> r̃) and over
    time:
                              Pt = ert P0
                           Pt+∆t = er∆t Pt
                                   ln Pt+∆t − ln Pt
 ...
 5. The internal rate of return (IRR) of the stream of cash flows,
    ...
    equal to zero:
              ...
              (1 + r)    (1 + r)    (1 + r)           (1 + r)T
                                                              ...
                                                              Revised: December 2, 1998
108                                          6.2. NOTATION AND PRELIMINARIES

                            W0 =         the investor’s initial wealth
                            W1 =         the investor’s desired expected final wealth
                            N  =         number of risky assets
                            rf =         return on the riskfree asset
                       r̃j ∈ < =         return on jth risky asset
                     r̃ ∈ <N   =         (r̃1 , . . . , r̃N )
              e = E[r̃] ∈ <N   =         vector of expected returns
                 V ∈ <N ×N     =         variance-covariance matrix of returns
                             1 =         (1, 1, . . . , 1)
                               =         N -dimensional vector of 1s
                       wj ∈ < =          amount invested in jth risky asset
                      w ∈ <N =           (w1 , . . . , wn )
                   r̃w = w> r̃ =         return on the portfolio w
                       W1
       µ ≡ E[r̃w ] ≡ W  0
...
Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                      109
...
as a w with w> 1 = W0 or as the corresponding normal portfolio, W10 w. It will
...
A number of further comments are in order at this stage.
   ...
   2. Since we will be dealing on occasion with hedge portfolios, we will in future
      ...
      is equivalent to the gross return. It is just w> r̃. The payoff on a zero cost
      portfolio can also be defined as w> r̃.
      Note that where we initially worked with net rates of return ( PP10 − 1), we
      will deal henceforth with gross rates of return ( PP10 ).

                                                          Revised: December 2, 1998
110              6.3. THE SINGLE-PERIOD PORTFOLIO CHOICE PROBLEM
...
Choose wj s to maximize expected utility of date 1 wealth,
                                             X                   X
                        W̃ = (W0 −                   wj )rf +          wj r̃j
                                             j                     j
                                             X
...
Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                             111


i.e.
                                                           X
              max f (w1 , . . . , wN ) ≡ E[u(W0 rf +              wj (r̃j − rf ))]
              {wj }
                                                             j

The first order conditions are:

                              E[u0 (W̃ )(r̃j − rf )] = 0     ∀j.                     (6.3.3)

The Hessian matrix of the objective function is:
                                h                                      i
                      A ≡ E u00 W̃ (r̃ − rf 1) (r̃ − rf 1)> .                        (6.3.4)

h> Ah < 0 ∀h 6= 0N if and only if u00 < 0. Since we have assumed that investor
...
Another way of writing (6.3.3) is:
                         ...
                         E[u0 (W̃ )r̃j ] = E[u0 (W̃ )]rf         ∀j,                 (6.3.5)

or             h          i
          Cov u0 (W̃ ), r̃j + E[u0 (W̃ )]E[r̃j ] = E[u0 (W̃ )]rf             ∀j,     (6.3.6)
or                                            h              i
                                         Cov u0 (W̃ ), r̃j
                        E[r̃j − rf ] =                             ∀j,               (6.3.7)
                                             E[u0 (W̃ )]

Suppose pj is the price of the random payoff x̃j . Then r̃j = x̃pjj and
...
In other words, payoffs are valued by taking their expected present value, using
                                   0 W̃ )
the stochastic discount factor E[uu0 ((W̃ )]rf
...
lem sets.)
                                                                   ...
                                                                   Revised: December 2, 1998
112            6.3. THE SINGLE-PERIOD PORTFOLIO CHOICE PROBLEM
...
⇐⇒
                            ...
                            E[u0 (W0 rf ) (r̃ − rf )] > 0                   (6.3.10)
⇐⇒

                            u0 (W0 rf ) E[(r̃ − rf )] > 0                   (6.3.11)
⇐⇒
...
the risky asset.
Definition 6.3.1 Let f : X → <++ be a positive-valued function defined on X ⊆
<k++ . Then the elasticity of f with respect to xi at x∗ is
                                   x∗i ∂f ∗
                                            (x ) .
                                  f (x) ∂xi
Roughly speaking, the elasticity is just ∂∂lnlnxfi , or the slope of the graph of the
...
Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                               113
...
Now consider other properties of asset demands (assuming that E [r̃] > rf ):
                                                           
                                                    da
   • DARA ⇒ risky asset normal                     dW0
                                                       >0
                             
                      da
   • CARA ⇒          dW0
...
which can be written:
                       ...
                       E[u0 (W0 rf + a(r̃ − rf ))(r̃ − rf )] = 0,                     (6.3.16)

we have
                            da   E[u00 (W̃ )(r̃ − rf )]rf
                               =                           .                          (6.3.17)
                           dW0   −E[u00 (W̃ )(r̃ − rf )2 ]

                                                                     Revised: December 2, 1998
114                 6.3. THE SINGLE-PERIOD PORTFOLIO CHOICE PROBLEM


By concavity, the denominator is positive. Therefore:

                        sign (da/dW0 ) = sign {E[u00 (W̃ )(r̃ − rf )]}        (6.3.18)

We will show that both are positive.
For decreasing absolute risk aversion:3

                              r̃ > rf ⇒ RA (W̃ ) < RA (W0 rf )
                              r̃ ≤ rf ⇒ RA (W̃ ) ≥ RA (W0 rf )

Multiplying both sides of each inequality by −u0 (W̃ )(r̃ − rf ) gives respectively:

                       u00 (W̃ )(r̃ − rf ) > −RA (W0 rf )u0 (W̃ )(r̃ − rf )   (6.3.19)

in the event that r̃ > rf , and

                       u00 (W̃ )(r̃ − rf ) ≥ −RA (W0 rf )u0 (W̃ )(r̃ − rf )   (6.3.20)

(the same result) in the event that r̃ ≤ rf
Integrating over both events implies:

              E[u00 (W̃ )(r̃ − rf )] > −RA (W0 rf )E[u0 (W̃ )(r̃ − rf )],     (6.3.21)

provided that r̃ > rf with positive probability.
...
due to ?.
   3
       Think about whether separating out the case of r̃ = rf is necessary.


Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                         115
...
i.e. ∀ portfolios p, wealths W0 , ∃λ s.t.
    h                                 i        h                        i
 E u W0 rf + λW0 p∗ > (r̃ − rf 1)           ≥ E u W0 rf + p> (r̃ − rf 1)        (6.3.22)
...
i.e. the utility function is of one of these types:
                               1
   • Extended power: u(z) = (C+1)B (A + Bz)C+1

   • Logarithmic: u(z) = ln(A + Bz)
                                    A
   • Negative exponential: u(z) = − B exp{Bz}

where A, B and C are chosen to guarantee u0 > 0, u00 < 0.
i.e. marginal utility satisfies

                  u0 (z) = (A + Bz)C        or u0 (z) = A exp{Bz}               (6.3.23)

where A, B and C are again chosen to guarantee u0 > 0, u00 < 0.
...
We will show that u0 (z) = (A + Bz)C is sufficient for two-fund separation.
...
or equivalently to the system of equations
            ...
            E[(1 +                                                              (6.3.27)
                      j   A + BW0 rf
                                                            ...
                                                            Revised: December 2, 1998
116                   6.4. MATHEMATICS OF THE PORTFOLIO FRONTIER


or
                                     xj (r̃j − rf ))C (r̃i − rf )] = 0
                                X
                      E[(1 +                                               (6.3.28)
                                j
...
Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                       117
...
just <N . However, introductory treatments generally present it (without proof) as
...
tion is not essential and will be avoided.
                                                           ...
                                                           Revised: December 2, 1998
118                    6.4. MATHEMATICS OF THE PORTFOLIO FRONTIER
...
To avoid degeneracies, we require:
   ...
   2. that the variance-covariance matrix, V, is (strictly) positive definite. We
      ...
      require this slightly stronger condition. To see why, suppose
                                ...
                                ∃w 6= 0N s.t. w> Vw = 0                         (6.4.6)

      Then ∃ a portfolio whose return w> r̃ = r̃w has zero variance.
      This implies that r̃w = r0 (say) w.p.1 or, essentially, that this portfolio is
      riskless.
...
We will call these columns g and h and write the solution as
                        ...
                        w = W0 g + W1 h = W0 (g + µh) .                         (6.4.7)

Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                      119
...
This allows the solution to be written as:
                                 γ          λ
                            w=     (V−1 1) + (V−1 e).                        (6.4.13)
                                 C          A
1
C
  (V−1 1) and A1 (V−1 e) are both unit portfolios, so γ + λ = W0 . We know that
...
of expected final wealth, the corresponding Lagrange multiplier, λ = 0. Thus
 γ
C
   (V−1 1) is the global minimum variance portfolio with cost W0 (which in fact

                                                            Revised: December 2, 1998
120                        6.4. MATHEMATICS OF THE PORTFOLIO FRONTIER


equals γ in this case) and C1 (V−1 1) is the global minimum variance unit cost
portfolio, which we will denote wMVP .
In fact, we can combine (6.4.7) and (6.4.13) and write the solution as:
                                                A
                                                     
                            w = W0   wMVP + µ −   h .                        (6.4.14)
                                                C
...
In <N , the set of unit cost frontier portfolios is the line passing thru g, parallel to
h.
It follows immediately that the frontier (like any straight line in <N ) is a convex
...
The portfolio weight vectors g and h are
                                1
                            g =   [B(V−1 1) − A(V−1 e)]                      (6.4.15)
                                D
                                1
                            h =   [C(V−1 e) − A(V−1 1)]                      (6.4.16)
                                D
We have
                                                   B
                               Var[r̃g ] = g> Vg =                           (6.4.17)
...
Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                      121
...
Since wMVP is collinear with V−1 1, it is orthogonal to all portfolios w for which
w> VV−1 1 = 0 or in other words to all portfolios for which w> 1 = 0. But these
are precisely all hedge portfolios, including h.
Similarly, any portfolio collinear with V−1 e is orthogonal to all portfolios with
zero expected return, since w> VV−1 e = 0 or in other words w> e = 0.
                                                                             Q.E.D.
...
tions for different µs (or W1 s) are the tangency points between these ellipses and
...
variance portfolio corresponding to W0 , W0 wMVP . A similar geometric interpre-
...
We can always choose an orthogonal basis for the portfolio frontier.
                                                          ...
                                                          Revised: December 2, 1998
122                       6.4. MATHEMATICS OF THE PORTFOLIO FRONTIER


For any frontier portfolio p 6= wMVP , there is a unique unit cost frontier portfolio
zp which is orthogonal to p.
Another
  h    i important exercise is to figure out the relationship between E [r̃p ] and
E r̃zp .
Any two frontier portfolios span the frontier, in particular any unit cost p 6= wMVP
...
the form fq + uq where
                         fq ≡ W0 (g + E[r̃q ]h)                                       (6.4.19)
                            = W0 (βqp p + (1 − βqp )zp ) (say)                        (6.4.20)
is the frontier portfolio with expected return E[r̃q ] and cost W0 and uq is a hedge
...
holds:
                     q = fq + uq = βqp p + (1 − βqp ) zp + uq                 (6.4.21)
...
We will now derive the relation:
                          E[r̃q ] − E[r̃zp ] = βqp (E[r̃p ] − E[r̃zp ])               (6.4.22)
   5
     Aside: For the frontier portfolio fq to second degree stochastically dominate the arbitrary
portfolio q, we will need zero conditional expected return on uq , and will have to show that
                                           
                             Cov r̃uq , r̃fq = 0 =⇒ E[r̃uq |r̃fq ] = 0

The normal distribution is the only case where this is true.


Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                                 123
...
assumptionsh about ipreferences.
                               h        i
Since Cov r̃uq , r̃p = Cov r̃zp , r̃p = 0, taking covariances with r̃p in (6.4.21)
gives:                                    h          i
                    Cov [r̃q , r̃p ] = Cov r̃fq , r̃p = βqp Var[r̃p ]      (6.4.23)
or
                                             Cov [r̃q , r̃p ]
                                     βqp =                                             (6.4.24)
                                              Var[r̃p ]
...
Taking expected returns in (6.4.21) yields again:
                         ...
                         E[r̃q ] = βqp E[r̃p ] + (1 − βqp )E[r̃zp ],                   (6.4.26)

which can be rearranged to obtain (6.4.22).

The Global Minimum Variance Portfolio
                   Var[r̃g+µh ] = g> Vg + 2µ(g> Vh) + µ2 (h> Vh)                       (6.4.27)
...
frontier.
The mean, µ, and variance, σ 2 , of the rate of return associated with each point on
the frontier are related by the quadratic equation:

                       (σ 2 − Var[w>                    >       2
                                   MVP r̃]) = φ(µ − E[w MVP r̃]) ,             (6.4.34)
...
Applying Pythagoras’ theorem to the triangle with vertices at 0, p and MVP yields:
                                                  ...
                                                  A
                                                    
                  2
                σ = Var[r̃p ] = Var[r̃MVP ] + µ −                  Var[r̃h ]   (6.4.35)
                                                  C

Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                            125


Recall from the coordinate geometry of conic sections that

                  Var[r̃p ] = Var[r̃MVP ] + (µ − E[r̃MVP ])2 Var[r̃h ]             (6.4.36)
...
Var[r̃h ] < 0 (impossible) gives a circle with center (1/C, A/C).
Var[r̃MVP ] = 0 (the presence of a riskless asset) allows the square root to be taken
on both sides:
                                           A q
                                            
                            σ =± µ−              Var[r̃h ]                   (6.4.43)
...
i.e. the conic section becomes the pair of lines which are its asymptotes otherwise.
                                                                  ...
                                                                  Revised: December 2, 1998
126                    6.4. MATHEMATICS OF THE PORTFOLIO FRONTIER


Portfolios on which the expected return, µ, exceeds w>      MVP e are termed efficient,
...
its expected return exceeds the minimum variance expected return A/C = E[r̃MVP ].
The set of efficient portfolios in <N (or efficient frontier) is the half-line emanat-
...
a frontier portfolio with expected return µ solves:
      ...
      Cov [r̃MVP + (µ − E[r̃MVP ])r̃h , r̃MVP + (µ0 − E[r̃MVP ])r̃h ] = 0    (6.4.44)

or, since r̃h and r̃MVP are uncorrelated:

         Var[r̃MVP ] + (µ − E[r̃MVP ])(µ0 − E[r̃MVP ])Var[r̃h ] = 0          (6.4.45)

To make this true, we must have

                         (µ − E[r̃MVP ])(µ0 − E[r̃MVP ]) < 0                 (6.4.46)

or µ and µ0 on opposite sides of E[r̃MVP ] as shown.
...
Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                       127


where we substituted for σ 2 from the definition of the frontier.
...
To find zp in mean-variance space, note that the line joining (σ 2 , µ) to the MVP
intercepts the µ axis at:
                     µ − E[r̃MVP ]               µ − E[r̃MVP ]
           µ − σ2    2
                                    = µ − σ2                                  (6.4.51)
                    σ − Var[r̃MVP ]          (µ − E[r̃MVP )2 Var[r̃h ]
...
gether at this stage.
   ...
   1. The variance minimisation solution from first principles.
      It can be seen that w is the solution to:
                     1
              min L = w> Vw + λ(µ − w> e) + γ(W0 − w> 1)                      (6.4.52)
             {w, , } 2
      ...
      e> and 1> and using the constraints yields:
                             ...
                             µ = λ(e> V−1 e) + γ(e> V−1 1)                    (6.4.56)
                             1 = λ(1> V−1 e) + γ(1> V−1 1)                    (6.4.57)

                                                           Revised: December 2, 1998
128                   6.4. MATHEMATICS OF THE PORTFOLIO FRONTIER
  ...
  2. Derivation of (6.4.22).
      ...
      Figure 3C goes here.
                                                               ...
                                                               E[r̃ −r̃zp ]
      We already saw that the outer frontier has slope √ p                    .
                                                                  Var[r̃p ]

      At the point on the inner frontier with wq invested in q and (1 − wq ) in p,

                µ = E[r̃p ] + wq (E[r̃q − r̃p ])                          (6.4.60)
                2    2
               σ = wq Var[r̃q ]
                    +2wq (1 − wq )Cov [r̃p , r̃q ] + (1 − wq )2 Var[r̃p ] (6.4.61)
      ...
      and rearranging yields:
                         ...
                         E[r̃q ] − E[r̃zp ] = βqp (E[r̃p ] − E[r̃zp ])            (6.4.66)

Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                    129
...
where all assets were risky:
                                                 µ − rf
                           wp = V−1 (e − rf 1)                             (6.4.69)
                                                   H
where

     H = (e − 1rf )> V−1 (e − 1rf ) = B − 2Arf + Crf 2 > 0 ∀rf             (6.4.70)
...
return
                    aE[r̃p ] + (1 − a)rf = rf + a(E[r̃p ] − rf )
                                    q
and standard deviation of returns a Var[r̃p ].
...
6.5 Market Equilibrium and the Capital Asset Pric-
    ing Model
6.5.1 Pricing assets and predicting security returns
...
Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                        131
...
6.5.2 Properties of the market portfolio
Let
      mj    = weight of security j in the market portfolio m
  i
 W0 (> 0)   = individual i’s initial wealth
      wij = proportion of individual i’s initial wealth
...
folio, and hence, ∀q, the CAPM equation
                    E [r̃q ] = (1 − βqm ) E [r̃zm ] + βqm E [r̃m ]              (6.5.4)
holds.

                                                              Revised: December 2, 1998
132                          6.5. MARKET EQUILIBRIUM AND THE CAPM


Theorem 6.5.2 All strictly risk-averse investors hold frontier portfolios if and
only if                        h         i
                             E r̃uq |r̃fq = 0 ∀q                         (6.5.5)
...
reasons in the definition of a frontier portfolio:
                      ...
                      E[r̃q ] = (1 − βqp )E[r̃zp ] + βqp E[r̃p ]            (6.5.6)
...
Since the market portfolio is then on the frontier, it follows that:
                     ...
                     E[r̃q ] = (1 − βqm )E[r̃zm ] + βqm E[r̃m ]             (6.5.7)
...
Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                      133
...
tangency portfolio, t, and hence, ∀q, the traditional CAPM equation
                       ...
                       E [r̃q ] = (1 − βqm ) rf + βqm E [r̃m ]               (6.5.12)
...
Assume that a riskless asset exists, with return rf < E[r̃mvp ].
...
then the market/tangency portfolio is efficient.
                                                          ...
                                                          Revised: December 2, 1998
134                             6.5. MARKET EQUILIBRIUM AND THE CAPM
...
portfolio with
                                 E[r̃] < rf                            (6.5.14)
for
                     E[u(W0 (1 + r̃))] ≤ u(E[W0 (1 + r̃)])                             (6.5.15)
                                       < u(W0 (1 + rf ))                               (6.5.16)
...
the first order conditions for the canonical portfolio choice problem:
             0 = E[u0i (W̃i )(r̃j − rf )]        ∀ i, j                                (6.5.17)
                                                       h                       i
                 = E[u0i (W̃i )]E[r̃j − rf ] + Cov u0i (W̃i ), r̃j                     (6.5.18)
                                                                           h       i
                 = E[u0i (W̃i )]E[r̃j − rf ] + E[u00i (W̃i )]Cov W̃i , r̃j             (6.5.19)
...
Revised: December 2, 1998
CHAPTER 6. PORTFOLIO THEORY                                                                           135
                        ...
                        E[r̃j − rf ]
                             ...
                             θi                k=1
                                                                         i
                                                                                  = W0m wmk by market-
                                                                 P
Summing over i, this gives (since we have                             i W0 wik
            P
clearing and k wmk r̃k = r̃m by definition):
                                I
                                         θi−1 ) = Wm0 Cov [r̃m , r̃j ]
                                X
                 E[r̃j − rf ](                                                                    (6.5.25)
                                 i=1
                                                          I
                                                                 θi−1 )−1 Wm0 Cov [r̃m , r̃j ]
                                                          X
            or             E[r̃j − rf ] = (                                                       (6.5.26)
                                                           i=1
...
We conclude with some examples.
   ...
   2. Quadratic utility:
      ...
      lemma.
                                                                                 ...
                                                                                 Revised: December 2, 1998
136                         6.5. MARKET EQUILIBRIUM AND THE CAPM




Revised: December 2, 1998
CHAPTER 7. INVESTMENT ANALYSIS                                                      137
...
7.2 Arbitrage and the Pricing of Derivative Securi-
    ties
7.2.1 The binomial option pricing model
This still has to be typed up. It follows very naturally from the stuff in Section 5.4.


7.2.2 The Black-Scholes option pricing model
...
whose price, S̃t , follows the stochastic differential equation:

                               dS̃t = µS̃t dt + σ S̃t dz̃t ,

where {z̃t }Tt=0 is a Brownian motion process; a bond, whose price, Bt , follows the
differential equation:
                                   dBt = rBt dt;
and a call option on the stock with strike price X and maturity date T .

                                                               Revised: December 2, 1998
138             7.2. ARBITRAGE AND PRICING DERIVATIVE SECURITIES
...
Let the price of the call at time t be C̃t . Guess that C̃t = C(S̃t , t). By Ito’s lemma:
...
Then we claim that the solution to the Black-Scholes equation is:
                                                            √ 
             C(S, t) = SN (d (S, τ )) − Xe−rτ N d (S, τ ) − σ τ ,
...
Revised: December 2, 1998
CHAPTER 7. INVESTMENT ANALYSIS                                                          139
...
Note also that
                          √      1 2            √
          N 0 d (S, τ ) − σ τ = e− 2 σ τ ed(S,τ )σ τ N 0 (d (S, τ ))                 (7.2.5)
                                                    S (r+ 12 σ2 )τ
                                                                
                                        1   2
                                 = e− 2 σ τ           e            N 0 (d (S, τ ))   (7.2.6)
                                                    X
                                      S rτ 0
                                 =      e N (d (S, τ )) .                            (7.2.7)
                                      X
  ...
  ∂t
      ...
      = SN 0 (d (S, τ ))          − Xe−rτ ×
                          ...
                          √                                            √
                                                   !                        !
                              ∂d (S, τ )       σ                       
        N 0 d (S, τ ) − σ τ                − √       + rN d (S, τ ) − σ τ
                                     ∂t        2 τ
                                                                        (7.2.8)
                          σ                            √  
   = −SN 0 (d (S, τ )) √ − Xe−rτ rN d (S, τ ) − σ τ                     (7.2.9)
                         2 τ
  ∂C
  ∂S
                      ∂d (S, τ )
...
Substituting these expressions in the original partial differential equation yields:
      ...
      ∂t   2 ∂S 2       ∂S
                                                               Revised: December 2, 1998
140                         7.3. MULTI-PERIOD INVESTMENT PROBLEMS

                                                                              !
                                       0        −Sσ 1       ∂d (S, τ )
      = N (d (S, τ )) (rS − rS) + N (d (S, τ )) √ + σ 2 S 2
                                                2 τ  2        ∂S
                            √       −rτ        −rτ
           +N d (S, τ ) − σ τ −Xe r + rXe                          (7.2.13)
      = 0.                                                         (7.2.14)
