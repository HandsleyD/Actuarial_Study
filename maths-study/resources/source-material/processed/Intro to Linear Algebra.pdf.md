---
normalized_id: shared-pdf-reference-intro-to-linear-algebra
exam_code: SHARED
material_scope: intro to linear algebra.pdf
material_group: shared
document_type: pdf-reference
source_repo_path: resources/source-material/shared/Intro to Linear Algebra.pdf
conversion_method: pdftotext
qa_status: pending
---
# shared-pdf-reference-intro-to-linear-algebra
...
The outer product of a and b is then
               ...
                2                                  2*4 2*3 2*5 2*7 2*9 
         abT =  5  [ 4       3    5   7     9 ] =  5*4 5*3 5*5 5*7 5*9 
                1                                  1*4 1*3 1*5 1*7 1*9 
                                   8    6 10 14 18 
                                 = 2 0 1 5 25 35 45                              (15)
                                   4    3  5  7  9 
...
operations to reduce A to echelon form:
                 ...
                  4 3 4 | 0 1 0  ➔  0 1 2 |- 2 1 0  ➔
                 -4 2 2 | 0 0 1     0 0 - 4 |10 - 4 1 
...
two dimensional subspace of R 3 , and is a vector space in its own right. In general, a
...
combinations of the vectors are nonzero
                ...
                c1v1 + c2v2 + ... + ckvk ≠ 0 unless c1 = c2 = ... = ck = 0          (47)
...
A set of vectors w 1, w 2, ... , w k, in R n is said to span the space if every vector v in R n
can be expressed as a linear combination of w’s, i.e.

                        v = c1w 1 + c2w 2 + ... + ckw k for some ci.                (48)

Note that for the set of w’s to span R n then lk≥ n. A basis for a vector space is a set of
...
For any m by n matrix A, the nullspace N (A) and the row space R (A T) are orthogonal
subspaces of R n. Likewise, the left nullspace N (A T ) and the column space R (A) are
...
then X TX is nonsingular and therefore invertible. This is a very important result as we
shall see shortly.
...
however, to minimize the squared length,
                      ...
                      E 2 = (xb - y)T(xb - y) = xTxb2 - 2xTyb + yTy                 (52)
...
of X, Xc. Thus, for all choices of c,
                      ...
                      (Xc)T (Xb^ - y) = 0, or cT[X TXb^ - X Ty] = 0                 (55)
...
rank(X T X) < n. However, what about when X is nearly rank deficient, i.e. when one
...
The quantity X(X TX)-1X T, which we shall denote P, is a projection matrix. Projection
...
                                                   for i ≠ j
                                 v iT v j =   { 10 for i = j                         (66)

Of course, we are most familiar with the standard basis, v 1 = [1 0 ... 0]T , v 2 = [0 1 ...
...
following properties:
                          ...
                          QTQ = I, QQT = I, and QT = Q-1                           (68)

Furthermore, Q will also have orthonormal rows, i.e. if Q is an orthogonal matrix, so is
Q T. We will see this again shortly when we consider Principal Components Analysis
...
that:
              ...
              X b^ equals the projection of y into the column space of X, and
